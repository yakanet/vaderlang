...
vader_string_t std_numbers_strip_numeric_text(vader_string_t l0, int32_t l1) {
    ...
    t1 = std_string_byte_contains(l5, 877u);
    if (!(t1)) {
        { vader_string_t __vret = l5; vader_gc_top = gc_frame.prev; return __vret; }
    }
    vader_struct___lambda_env_6048_t* _a1_obj = (vader_struct___lambda_env_6048_t*) vader_gc_alloc(sizeof(vader_struct___lambda_env_6048_t));
