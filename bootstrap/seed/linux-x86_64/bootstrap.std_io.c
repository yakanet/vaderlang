...
vader_string_t std_io_current_executable_location(void) {
    ...
    t3 = vader_host_system_posix_sys_readlink(453u, l0, l1);
    if ((t3 <= INT64_C(0))) {
...
static vader_box_t std_io_read_file_bytes(vader_string_t l0, size_t l1) {
    ...
    vader_gc_top = &gc_frame;
    l2 = vader_host_system_posix_sys_fopen(l0, 1968u);
    if (l2.tag == 0u) {
        t0 = std_io_last_io_error(1358u);
...
static vader_box_t std_io_write_file_bytes(vader_string_t l0, void* l1) {
    ...
    l4 = vader_host_system_posix_sys_fopen(l0, 2410u);
    if (l4.tag == 0u) {
