...
- [ ] **Rethink the portable API and the FFI for multiple targets** (added 2026-08-27) — a DESIGN question, deliberately not a refactor. Today's shape only works because there is effectively one target family. With wasm, js, windows, linux and darwin (§3.10 already commits to WASM) it does not scale, and the pressure showed up as bugs before it showed up as a design discussion.
  ...
  - **Independent of the design, and available today**: the `_WIN32` branches are exercised by nothing but CI, where `test-windows` is `continue-on-error: true` and can rot silently. Cross-compiling with mingw-w64 and running under wine works — `wine vader.exe test vader/hooks` runs the whole module, and it verified both fixes above. ⚠️ But wine is a **regression net, not a Windows oracle**: its `GetFileAttributesA("C:")` succeeds, so it would NOT have caught the drive-root bug. Worth knowing before building a job on it and believing Windows is covered.
