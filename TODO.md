...
- [ ] **Transitive module loading + per-module dump sections** — `load_project` walks entry + depth-1 imports only. TS walks transitively. The snapshot for `namespace_alias_dedupe` lists every reachable module (`## vader/diagnostics/codes.vader`, `## vader/lexer/lexer.vader`, …, ~14k lines). Even after fixing P.Decl, the snapshot will diff on the missing module sections. Fix : load_project queues every import from every loaded module until fixed point. Then `collect_direct_imports` already walks `per_module` so the dump should fall into place.
  ...
  - **Caveat on the audit methodology** : the "TS-side dump emits 0 errors" comparison was misleading. Running `bun src/index.ts dump --stage=typed-ast <file>` on a single stdlib file silently bails before typecheck because the resolver can't find the project root from a single-file path — TS emits an empty `# Typecheck` header and 0 diagnostics regardless of whether the code is well-formed. The Vader-side diagnostics are still real and worth fixing, but a future audit should drive both compilers from the snippet/parity infrastructure (which sets up the project context correctly) or run TS with an equivalent project-root resolution.
...
- [x] **UFCS dispatch on Array → Iterator receiver** (2026-05-18, commit `db055dc2`) — `try_curry_receiver` now mirrors TS's `rankOverloadsByFirstParam` flexibility : direct `is_assignable`, wildcard TypeParam, or trait widening (incl. the hardcoded `Array(T) → Iterator(T)`). Closed 8 errors in `stdlib/std/iter.vader` test_any/test_all/test_find/test_find_map.
- [x] **Missing bounds on `MutableMap` accessors** (2026-05-18) — `put` / `get` / `contains_key` were declared `fn[K, V](...)` without `K: Hash & Equals`, so their bodies' `key.hash()` / `key.equals(...)` correctly errored T3008. Both TS and Vader emit on bound-less `T.method()` — the "TS = 0 errors" we measured during the 2026-05-18 audit was misleading because TS silently bailed on single-file stdlib dumps (no project root, imports unresolved → typecheck skipped). Adding `K: Hash & Equals` to the three accessors closed the 6 collections.vader T3008s.
