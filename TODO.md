...
- [ ] **`std/iter` short-circuiting combinators** (`any` / `all` / `find` / `find_map`). Today's stdlib ships only non-short-circuiting (`map` / `filter` / `fold` / `take` / `skip` / `sum` / `count` / `collect` / `zip` / `chain` / `enumerate` / `flat_map`). Predicate-search shapes recur every time a loop "stops on first match" is what the caller wants ; today the only options are a hand-rolled `for ... if return true` loop or `fold` (loses early-exit). ~5 lines per fn, both eager and lazy flavours. `arr.find(p)` returns `T | null`, consumers narrow naturally via flow-narrowing.
...
- [ ] **Byte literal syntax `b'{'`** — every byte-level comparison in `vader/lexer/lexer.vader` and `vader/lsp/indexer.vader` writes `b == u8(u32('{'))` (triple cast). Two routes : (a) Rust-style `b'{' :: u8 = 0x7B` parser literal, (b) typechecker rule allowing direct `char→u8` cast when the codepoint is < 128 (traps at runtime for non-ASCII). (a) is cleaner — explicit intent, no hidden fallible cast.
...
- [ ] `@size_of(t)` where `t: type` is comptime-evaluable.
- [ ] Test snippet.
...
- [~] **Hash-cons `Type` values + re-key registries by reference identity** (Phase A + B step 1 landed 2026-05-11). Done : `MonoEntry.id` / `isMain` split off `mangled` ; lookups re-keyed on `canonicalArgsKey`. Constructors `mkStruct`/`mkTrait`/`mkArray`/`mkTuple`/`mkFn`/`mkUnion`/`mkTypeParam`/`mkEnum` with per-kind cache + global `internId` WeakMap in place. **Remaining (B step 2)** : migrate the ~53 inline `{ kind: "Struct", ... }` Type literals across `src/typecheck/passes/*.ts` and `src/lower/passes/*.ts` to use the constructors. Mechanical, ~½ day. Once landed : `equalsType` short-circuits on `a === b`, `canonicalArgsKey` collapses to integer concat, Map-over-Type becomes safe.
...
- [ ] **Allow `int_type(enum_value)` cast for `enum(IntT)` types.** Today `inferTypeConstructorCall` rejects enum as a cast source ; `vader/lsp/log.vader:level_code` exists purely as a workaround. Allow `i32(MyEnum.Variant)` when the target is assignment-compatible with the enum's declared `repr`. Mirrors Go / Rust `as` / Zig `@intFromEnum`. Drop `level_code` once landed.
- [x] Implicit `void` return — already worked ; added `void` as reserved ident (R2019).
- [ ] **`Iterable($T)` trait + auto-`.iter()` in `for-in`** — collections implement `Iterable(T)` once and `for x in target` desugars to `for x in target.iter()` whenever the target implements `Iterable(T)`. Drops `iter :: fn(self: MutableList) -> ...` boilerplate. SPEC §7 *for*.
- [~] **Trait-object boxing + dynamic dispatch for `Iterator(T)` (and other traits)** (initial 2026-05-08, partial regression observed 2026-05-11). Vtable IR is wired through bytecode/VM/C-emit. **However :** `tests/snippets/trait_box_range_iter/vm.snapshot` documents a compile error `T3009 next on Iterator($T)`, and `count(0..<5)` traps at `vm: reached unreachable`. The inference path that worked at the time of the original entry has regressed (or never fully covered the call site). Storing a trait-typed value in a struct field works — that's why `std/iter`'s lazy combinators ship with the struct-literal-direct pattern instead of fn helpers. Reopen and investigate before relying on `fn[T](it: Iterator(T))` shapes. `tests/snippets/iter_lazy/` shows what works ; `trait_box_range_iter` is the reference for what should work.
