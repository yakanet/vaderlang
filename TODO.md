...
- [ ] **A single-dash CLI token is swallowed instead of rejected** (noted 2026-07-27). `parse_known` keeps unknown `--…` tokens in `positionals` for layered parsing, and `first_file_arg` (`vader/cli/main.vader`) recovers the file argument by skipping anything that `starts_with("--")`. A token like `-o` starts with one dash, so it never reaches either filter: it lands in `positionals`, isn't the first entry, and is dropped without a word. `vader build x.virt --target=c -o out` therefore exits 0 having written `x.c` next to the input — the `-o` had no effect and no diagnostic, while the same command with `--help` correctly says `unknown flag --help`. (Found by writing `-o` out of habit; the real flag is `--out=<path>`.) Fix: treat a leading `-` the same as `--` in the passthrough / file-argument split, so an unrecognised short token surfaces as an error rather than vanishing. Decide at the same time whether short aliases (`-o` for `--out=`) are wanted at all — today none exist.
