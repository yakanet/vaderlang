...
- [ ] **Cross-module `const` reference crashes the VM (`local.set on empty stack`)** (added 2026-06-18, surfaced while writing the README hero snippet). Referencing a `const` imported from **another module** — e.g. `import "std/math" { pi }` then using `pi` in an expression — traps the native self-hosted VM at runtime : `vader vm: local.set on empty stack at pc=32 in 'main$main'` (exit 1). Reproduces on the freshly-built native `dist/.../vader run`. Tight bisection (each program run **alone in its own directory** to dodge the multi-`main` entry-selection trap below) : (S1) `import "std/math" { pi }` + use `pi` → **crash** ; (S4) same program but `pi :: 3.141592653589793` declared **locally in-module** + used → works (`24.566370614359172`, correct full precision) ; (S5) `import "std/math" { pi }` present but **unused** (literal in the body, so `pi` is DCE'd) → works. So the trigger is specifically *an imported const that survives DCE because it's referenced*. Matches the long-documented **`LoweredConstDecl` Phase-1 stub** (`vader/bytecode/emit.vader:556`, CLAUDE.md §9 divergence #2) — cross-module const refs land on a stale fn-table slot whose body is `unreachable`, so the value never reaches the stack and the following `local.set` underflows. Existing diag snippet : `tests/snippets/_diag_const_string/` (string-const variant). **Next step :** implement `LoweredConstDecl` accessor emission (the TS emitter emits an accessor `call`) so a referenced cross-module const materialises its value ; add a numeric-const `tests/snippets/_diag_*` repro. **Note :** this is also why `vader run <file>` printed surprising output during the investigation — `vader run` loads the **whole containing directory** as a project, so several single-file programs sharing a folder (each with its own `main`) resolve to one arbitrary entry (first alphabetically), not the file named on the command line. Not a cache. Worth its own follow-up : `vader run <file>` should run *that file*'s `main`, or at least warn on multiple-`main` ambiguity. **README follow-up :** the `README.md` hero snippet uses a local `pi :: 3.141592653589793` const as a workaround ; once this is fixed, restore it to the intended idiomatic form `import "std/math" { pi }`.
