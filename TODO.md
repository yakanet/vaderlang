...
- [ ] **Code-size : same-offset shortcut** — the lowering currently emits an O(N-variants) `if is V1 { (V1) e.f } else if is V2 ...` cascade per access. For wide unions like `Expr` (25 variants) this can blow up the bytecode when many call sites read the same common field — `bun src/index.ts build vader/cli/main.vader` tripped the JS engine's recursion limit when `expr_span(e)` (one cascade in a helper) was inlined to `e.span` at ~20 call sites (~500 nested `if`s in the parser body). Mitigated today by keeping the helpers (one cascade per helper, one fn-call per call site), but the proper fix is for the lowerer to detect that all variants store the field at the same struct offset and emit a single bare `LoweredFieldAccess` (no tag dispatch) — common for AST-style structs whose first field is uniformly `span`. Cross-check : the bytecode emit's `field` ops already work this way as long as the runtime layout is consistent ; the lowerer just needs to query the BcType offset table.
- [ ] **Peephole spread bug uncovered** — `applyLocalRules` in `src/bytecode/peephole.ts` used `body.push(...out)` which trips a RangeError when `out` exceeds the engine's spread-arg limit (~16k). Replaced with an explicit `for ... push` loop. The cascade-explosion above was what surfaced this ; the fix stays even after the same-offset shortcut lands so other large-body fns don't regress.
