...
- [ ] **`match expr.field { _ -> use expr.field }` doesn't narrow on a field-expr scrutinee** — surfaced 2026-05-14 migrating `vader/fmt/printer.vader` and `vader/parser/dump.vader` to the new `name :: import "..."` syntax. The new `NamedNamespaceImport.restricted : ImportName[] | null` field can't be consumed via the natural form `match nn.restricted { is null -> {} ; _ -> use nn.restricted as non-null }` — the typechecker still reads `nn.restricted` as `ImportName[] | null` in the wildcard arm and emits T3001 "expected `ImportName[]`, got `ImportName[] | null`". The narrowing flow only updates local-binding types, not field-expr reads in the same branch. Workaround : extract `r :: nn.restricted` before the match and operate on `r`. Same root cause as the §1.4 entry on `if v is T { v.field }` ; touches `src/typecheck/passes/match.ts` (arm narrowing) — propagate the narrowed type from `match`-scrutinee position to field-expr reads keyed by the same `(target.symbol.id, field)` pair within the arm.
