...
- [ ] Slot reuse (live-range analysis) — every let gets a fresh slot today. Acceptable until perf signal says otherwise. **Attempt 2026-05-16** : TS impl with back-edge extension for `loop` worked on the TS VM (`tests/vm.test.ts` green, ~17 % snapshot bytecode shrink) but broke the self-host Vader VM (`tests/vader_vm.test.ts` infinite loop on `custom_iter`, `std_regex`). Root cause hypothesised (audit 2026-05-16) at `vader/vm/exec.vader:169` `build_callee_frame` — `slots` array sized from `callee.local_count` ; mismatch when the bytecode references compacted slot indices that the metadata doesn't track. Re-attempt requires re-applying the pass in a dedicated debug session against `custom_iter` + `std_regex` to confirm the hypothesis and ship the fix (probable : either tighten the TS-side pass to update `local_count` correctly, or add a load-time validation in `vader/vm/parser.vader`). Sketch lives in commit history (search for `bytecode/liveness.ts`).
...
- [ ] **Inline `Iterator.next()` body at the for-in site** (bench/map_iter motivator). Today `for entry in m` over a `MutableMap` lowers to a loop calling `MapIterator.next()` per iter — bucket walking + chain-cursor reads dominate the runtime (~22 ms vs Go's 8 ms on the bench). Plan : at the lowerer, when the iter's static type is a concrete struct whose `Iterator.next()` body is a single-arm switch (e.g. `match self.chain_cursor { is Entry as e -> {…} is null -> {…} }`), inline the body into the for-in loop and let the surrounding pattern-match decision tree fold with the consumer. Same trick the iter-chain fusion already does for `MapIterator { source: FilterIterator { source: Range, … } }` ; generalise to non-range bottoms. Expected gain : map_iter Vader-vs-Go gap shrinks from 2.8 × to 1.5-2 ×. ~½ day. Open : how to express "inline-eligible body" without a heuristic explosion ; what to do when next() reads multiple fields (Entry, MapIterator are both 3-field structs).
- [ ] **Open-addressed hash table for `MutableMap`** (companion to next()-inline). The chaining design (`buckets: (Entry | null)[]` with linked-list `next` pointers) costs an extra ref deref per probe + an extra alloc per insertion ; open addressing (linear probing with Robin Hood + power-of-two sizing) keeps everything in one flat array, halves the per-lookup ref hops, and removes the `Entry.next` allocation entirely. Trade-offs : (a) Robin Hood is ~80 LoC of Vader vs the current ~50, (b) tombstones on delete (current chaining naturally compacts), (c) iterator stability across rehash (callers must complete iteration before any insert). Most likely lands together with next()-inline since both target map_iter's hot path. Expected combined gain : Vader within ~1.2 × of Go on map_iter. **Attempts 2026-05-16** : 3 sessions, 5 compiler bugs surfaced + 4 fixed (commits `12a8de68`, `9489305d`, `4d43ecb2`, `52164253`) : dedup struct/trait types by source position, cross-module ident resolution, cross-module typed-program swap for foreign sub-trees, FreeInt source pinning to cast target width. Robin Hood impl + Vader-side iter rewrite works end-to-end (snippets green VM + native, resize handled correctly). **5th bug unfixed** : a generic struct's field default that itself references the struct's TypeParams (`MutableSet :: struct[T] { inner: MutableMap(T, bool) = MutableMap(T, bool) {} }`) — the foreign-module-swapped lowering uses EMPTY_SUBST and doesn't substitute `T → concrete`, so `MutableMap(T, bool) {}` lowers to `struct.new on non-struct type` at runtime. Requires threading the caller's substitution into `lowerExprMaybeForeign` (subst keyed by definer's TypeParam symbols, not caller's — needs a name-based mapping or a TypedAST-aware specialise pass for defaults). Robin Hood stays reverted until this lands.
