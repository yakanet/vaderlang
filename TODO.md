...
- [ ] **Add `TY_STRING` / `TY_NULL` singletons (drop the per-literal alloc)** (surfaced 2026-07-01 by the `infer_*_lit` /simplify efficiency pass). `mk_primitive` is a plain factory — NOT interned (`vader/types/types.vader:171`, header confirms "no hash-consing for MVP") — so the string/null literal arms (`expr.vader:101,110` + the 7 `*_check_stub` string arms) allocate a fresh `PrimitiveType` per literal node typed, on every typecheck pass. The int/float/bool/char arms already return shared singletons (`TY_FREE_INT`/`TY_BOOL`/…). Mirror `TY_BOOL` (`types.vader:206`): add `export TY_STRING :: PrimitiveType { .name = "string" }` + `TY_NULL`, return them from those arms. Sound — `PrimitiveType` is compared structurally (`canonical_key`/`equals_type`), never by reference identity, and `TY_BOOL`/`TY_VOID` already prove the shared-singleton pattern. Aligned with the alloc-reduction chantiers. Reseed.
