...
- [ ] **`std/json` v2 — generated serialisers, and a plug-and-play way to connect them** (expanded 2026-08-27, user request). Today the surface is `JsonValue` + `parse` + `stringify` / `stringify_pretty`: struct ↔ JSON is hand-written by the caller. The goal is generated `to_json` / `from_json`, and — the part that matters — **a library ships its generator and a project connects it, rather than every project rewriting the generator**.
  ...
  - ⚠️ **Layering constraint, measured: the stdlib imports nothing from `vader/`.** A generator must read `AST.StructDecl`, i.e. `vader/parser` — so `std/json` cannot ship one without the stdlib gaining a dependency on the compiler, and a *companion* module under `std/` would only move that dependency, not remove it. **The resolution is a public build-time API belonging to neither** — see the driver-API item in §3.7. A generator then lives in its own namespace, depends on that API rather than on the compiler, and is never in a program's runtime closure.
...
- [ ] **Rethink the portable API and the FFI for multiple targets** (added 2026-08-27) — a DESIGN question, deliberately not a refactor. Today's shape only works because there is effectively one target family. With wasm, js, windows, linux and darwin (§3.10 already commits to WASM) it does not scale, and the pressure showed up as bugs before it showed up as a design discussion.
  ...
  - **Independent of the design: the Windows coverage gap is a policy one, not a missing job.** CI already runs a real `windows-latest` — the workflow says so and calls it "faithful (unlike wine)". What lets the `_WIN32` branches rot is `continue-on-error: true` on that job, whose exit condition is already written beside it: "flip it off once green". Three `vader/hooks` tests are what stands between here and there.
...
- [ ] **`dist/` layout: `lib/` beside `stdlib/` reads as two conventions** (added 2026-08-27). The bundle is `vader` (binary) + `stdlib/` + `runtime/c/` + `lib/vader/`: two *module* roots at different levels, and the `lib/` level exists for a mechanical reason rather than a principled one — the bundled binary is itself named `vader`, so the compiler's sources cannot sit at `<bundle>/vader/` without colliding with it (`vader/cli/driver.vader::BUNDLED_SOURCE_DIR`). One root would read better: `bin/vader` + `lib/std/…` + `lib/vader/…` + `lib/runtime/c/`. Touches `resolver::resolve_sidecar`, `compiler_source_root`, `bootstrap/build.sh` **and** `bootstrap/build.ps1` (which has its own `-Dist` block — easy to forget), plus the README's layout paragraph. Worth folding into the next change that moves anything in the bundle.
- [ ] **Split the driver-facing API from the human front-ends, and ship only what a driver needs** (added 2026-08-27). `--dist` copies all of `vader/` — **4.4 MB, 8× the 548 KB stdlib** — so a `build.vader` can `import "vader/hooks"`. Measured: `vader/lsp` (448 KB / 9532 lines), `vader/fmt` (112 KB / 2825), `vader/cli` (128 KB / 2583) and `vader/dap` (28 KB / 492) are **716 KB, 16 %** a driver never touches, and nothing outside those four imports them — so lazy resolution already keeps them out of a driver's closure. **The split is therefore a payload win, not a compile-time one**, and the distinction matters: the 8.2 s driver compile (274 037 lines of emitted C, 13 MB) is `vader/hooks` → `vader/pipeline` → resolver / typecheck / lower / midir / bytecode / c_emit — the compiler core, which a driver genuinely needs because `emit_native` builds the project. Cutting THAT would mean the driver calling the `vader` binary instead of linking it, i.e. the subprocess architecture the metacompilation plan rejected (a hook must share one compilation unit: type indices and atom ids are per-unit). Two separable pieces: **(a)** ship only the driver closure in `--dist` — bundle −16 %, no risk; **(b)** give the driver a **public API in a namespace that belongs to neither the compiler nor the stdlib** (user, 2026-08-27) — not a boundary drawn inside `vader/`, which would still make every consumer depend on compiler internals. That is what lets a *library* ship a build-time generator (see `std/json` v2 in §3.4) without the stdlib depending on `vader/` or the library depending on the compiler. It also decides what `--dist` ships: that namespace, not all of `vader/`. The enabler is already an open item — **Multi-library namespaces** below. Note what is left of that work: the *resolution*-level special-casing is gone (`is_stdlib_path` removed), but **four sites still hard-code the `std/` prefix for policy**: `resolver/module.vader:29`'s `STDLIB_PREFIX`, read by `typecheck/dead_code.vader:600` (lint scope); a literal in `typecheck/orchestrate.vader:463` (skips stdlib imports in a per-module walk); and two in the LSP (`analysis/resolver.vader:158`, `actions/organize_imports.vader:199`). Each would have to answer "is this namespace stdlib-like?" rather than "does the path start with `std/`".
    - ⚠️ **The crux to settle first: what shape does the AST take in a public API?** Re-exporting `vader/parser` makes the compiler's 55-variant AST public surface, so every internal change becomes breaking. A narrower view — what a generator actually needs (declaration kind, names, field names, a field's type as text or as a small closed enum) — stays stable but is lossy, and the JSON prototype already reads `StructField.ty` as a `TypeExpr` to tell `i32` from `string` from a sibling struct. Decide the fidelity/stability trade before naming the namespace.
- [ ] **Multi-library namespaces** — the `std/` prefix is still hard-coded at four policy sites (updated 2026-08-27: `resolver/module.vader:29`'s `STDLIB_PREFIX` read by `typecheck/dead_code.vader:600`, a literal in `typecheck/orchestrate.vader:463`, and two in the LSP; the *resolution*-level `is_stdlib_path` special-casing is already gone). Generalise to a registry of `prefix → on-disk root` fed by `vader.json` (e.g. `{ "libraries": { "web": "./vendor/vader-web", "experimental": "./libs/exp" } }`). Use cases : (a) third-party libs via a package manager, (b) compiler-private namespace (`internal/`) with a "stdlib + compiler only" rule, (c) workspace monorepos. Touches the four sites above plus the dump-stage filters. ~150 lines + manifest schema bump. (The old note named `resolveImportPath` / `isStdlibModule` — TS-era names that no longer exist.)
...
- [ ] **CI review** (added 2026-08-27, user request). Concrete gaps, measured on runs #208-#214:
  ...
  - **`test-windows` is `continue-on-error: true`**, so its failures never block. Exit condition already written beside it ("flip it off once green"); cross-ref the portable-API item above for what stands in the way.
