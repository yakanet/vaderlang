...
- [ ] **Expression-bodied impl methods don't get their return type inferred** (2026-05-25). `inferExprBodiedReturns` in `src/typecheck/check.ts:209` walks top-level `FnDecl`s only — `ImplDecl` members with `fn(...) = expr` shape and no `-> T` annotation stay at `Fn(..., Unresolved)` in `declTypes`. Stayed latent for a long time because the old `println(msg: string)` accepted Unresolved args silently. Surfaced when `println` became `fn[T: Display](msg: T)` — `tests/snippets/enum_trait_self_return` now traps because `Direction.to_string()` returns `?`, T can't be inferred at call-site, no mono → `unreachable`. Snippet's vm.snapshot was rewritten to record the trap and `tests/native.test.ts` skips it via the `# runtime error` marker. Fix : extend the settle loop to walk every `ImplDecl.members` too, threading the impl's `forType` as `selfType` into the body-check call.
