...
- [ ] **LSP : member completion suggests the wrong members** (noted 2026-06-20). On `f.mint()` where `f: NodeIdFactory`, the `textDocument/completion` popup offers `next` (detail `usize`) instead of `mint` (or the full member set). Observed in `vader/parser/assign_ids.vader::walk_fn_decl`. Likely the member-completion (`vader/lsp/handlers/completion.vader`, T2 struct-fields + impl-methods) resolves the receiver type's methods from the wrong table or a stale/partial impl set — cross-ref the known T2 limitation "methods from cross-module (stdlib) impls aren't offered (registry only carries entry-module impls)" in the LSP submodules plan. Investigate which impl-method table member completion reads for a receiver whose type's impls live in another module.

- [ ] **Typer : `lookup_fn_symbol_by_span` matches spans file-blind** (found 2026-06-20 while landing T3053). `vader/typecheck/decl.vader:391-392` matches a decl to its overload Symbol by `(line, column)` only, ignoring `file`. Two fns at the same `(line, col)` in **different sibling files of one module** (e.g. two `main`s at 1:1) collide → the second's `declare_fn` returns the first's Symbol → `decl_types[second.id]` is never populated (left `?`). This is the exact multi-file span hazard `span_key` was fixed for (folds in the file FNV-hash, `vader/diagnostics/diagnostic.vader:35-39`), but this call site was never updated. Effect today: any *legitimate* cross-file overload set sharing `(line, col)` mistypes one member ; it's also why T3053's redefinition check must read signatures from the AST, not `decl_types`. Fix : add a `same_file(sp.start.file, decl.name_span.start.file)` discriminator. Then T3053 could optionally simplify back to reading `decl_types` directly.
...
- [ ] **`vader run <file>` runs an arbitrary directory entry, not the named file** (split out 2026-06-19 from the cross-module-const item). `vader run <file>` loads the **whole containing directory** as a project, so several single-file programs sharing a folder (each with its own `main`) resolve to one arbitrary entry (first alphabetically), not the file named on the command line. Not a cache. `vader run <file>` should run *that file*'s `main`, or at least warn on multiple-`main` ambiguity. (Surfaced during the cross-module-const bisection — each repro had to run alone in its own directory to dodge this.)
