...
- [ ] **A modifier list's exclusions don't reach the UFCS table — `import "p" { x as _ }` leaves `recv.x()` callable** (found 2026-08-02, split out of the alias fix above). `push_module_exported_fns` pushes every public fn of a wildcard-imported path, and the first block of `populate_imported_fn_overloads` pushes a rename under its SOURCE name as well as its local one. So after `import "p" { bump as _ }` — or `{ bump as boost }` — `bump(c)` is correctly R2006 while `c.bump()` still resolves. Same divergence as the alias case, same table.

  **Attempted and reverted; the reason is the value here.** Filtering `push_overload` by `collected.wildcard_excludes[path]` closes both cases and breaks exactly one site: `vader/lsp/main_entry.vader:21` calls `parse(argv…, LSP_FLAGS)` under its own `import "std/cli"` (bare wildcard), but its SIBLING file `vader/lsp/main.vader:22` writes `import "std/cli" { parse as _ }` to dodge an R2031 against `std/json`. `merge_collected` (`vader/resolver/loader.vader:441-449`) unions the excludes across a module's files, so one file's exclusion silences another's legitimate wildcard — and SPEC §"Modules" says the opposite: *"`import` declarations are file-scoped. Names bound in one file of a multi-file module are not visible in sibling files"*. The union is already wrong; the filter just made it observable.

  **So the fix is a granularity change, not a filter.** `imported_fn_overloads` hangs off `TypedProgram`, which is per-MODULE; honouring file-scoped imports needs it keyed per FILE (the file is recoverable at every consumer — `Span.start.file`). Consumers to thread: `field.vader::{ufcs_name_has_candidates, try_free_fn_ufcs_with_first, ufcs_methods_for}` and `call.vader::{primary_overload_ft, collect_overload_candidates}`; producer-side, `CollectedModule` must keep its wildcard paths + excludes per file instead of merging them. Same granularity gap as the `files[0]`-only body walker (§1.13d) and as unused-imports (W0006) — worth doing as one chantier rather than three. Until then the module-wide table is documented as such at its definition.
