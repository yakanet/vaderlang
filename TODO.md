...
- [ ] **Relax `inLoop` filter in escape analysis** (follow-up surfaced by the SSA decision). Today escape analysis disqualifies every alloc inside a natural loop body, citing the C-emit's block-scoped storage aliasing across iterations. Empirically this means **0 stack-promotions on real code** (mandelbrot, brainfuck, mowitnow combined : 0/500+). Investigate two paths : (a) emit the storage in a per-iteration `for { … }` C scope so each iteration is a fresh frame, (b) accept iter-aliasing for locals that don't escape the iteration. Either unlocks real stack-promotion wins ; pick the simpler one.
