...
- [ ] **Numeric literal context-sensitivity for `usize` (and other primitives)** — 185 occurrences of `usize(0)` / `usize(1)` / `i + usize(1)` across `vader/`. Today integer literals default to `FreeInt` which resolves to `i32` ; assigning / comparing / passing them to a `usize` slot requires an explicit cast. Picks two shapes that would both work :
  - (a) **Comptime-int à la Zig** — a `FreeInt` literal flowing into a `usize` slot defaults to `usize` instead of `i32` (matches the slot's expected type). Already partly done for `FreeInt` → `i64`/`u64` (cf. §1.13c first bullet, 2026-05-07) ; extend to `usize` and the unsigned width family.
  - (b) **Short integer suffixes** — `0u` / `1u` for usize, `0us` / `1us` longer form. Less invasive but only papers over the surface.
  Preferred : (a). Touches `src/typecheck/passes/expr.ts:inferExpr` (free-int default rules) and `defaultIfFree`. Worth measuring how many of the 185 vanish under (a) vs (b) before picking. Visible cost lines : `vader/fmt/printer.vader:115-123`, `vader/parser/parser.vader:60-70`, `vader/lexer/lexer.vader` (many).

- [ ] **Flow-narrowing on `T | null` inside `if x != null { ... }`** — 50+ occurrences of `match opt { is null -> {} _ -> { use opt.field } }` across the self-host (`vader/fmt/printer.vader` × 26, `vader/lsp/indexer.vader` × 9, `vader/parser/parser.vader` × 5, etc.). The pattern is purely a workaround : the typechecker doesn't narrow `T | null` to `T` inside an `if`-true branch on `x != null`. Lowest-cost fix : extend the existing match-side narrowing (`MutableTyped.narrowed`, populated by `inferMatch`) to also fire on `BinaryExpr(.Neq, ident, NullLit)` conditions. Covers ~80% of the cases without designing `if let`. Optional follow-up later : null-coalescing `??` or `?.` chains — but probably unnecessary once narrowing lands. File : `src/typecheck/passes/expr.ts:inferIf` + the narrowing stack threading already used for matches.

- [ ] **Common-field access on union-aliases (`UnionFieldResolution` for `:: type` unions)** — `vader/fmt/printer.vader:241-271` and `vader/lsp/indexer.vader:430-460` are **the same 25-arm dispatch** returning `x.span` for every `Expr` variant. The TS typechecker already supports common-field on discriminated unions (cf. §1.18d completed, `typed-ast.ts:UnionFieldResolution`) — but the self-host AST defines `Expr` as a *`:: type` union-alias*, and that path doesn't trigger the same resolution. Confessed in `vader/fmt/printer.vader:4-7`. Likely a small extension to `inferField` to also handle `Type.kind === "UnionAlias"` (or whatever the internal name is) when every aliased variant carries the same-named field of the same type. Eliminates two redundant 25-arm fns in `vader/` and unlocks generic AST visitors going forward.
