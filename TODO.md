...
- [ ] **Namespace-FieldExpr in type position (`P.Decl`)** — `type_from_expr`'s FieldExpr arm reads `resolved.idents[span_key(e.span)]`, but the resolver doesn't bind the FieldExpr's full span (only the target IdentExpr `P`). For value position, `try_namespace_field` in `field.vader` handles it via `typed.namespace_exports`. Same needs to happen for type position. Fix : either thread `typed: TypedProgram | null` into `type_from_expr` (59 callers, most already have `typed`), or add a `namespace_member_types: i32 → string → Type` side-table on `ResolvedModule` that the orchestrator populates after `namespace_exports` builds. Blocks `via_folder (fn(d: P.Decl))` in `namespace_alias_dedupe`.
