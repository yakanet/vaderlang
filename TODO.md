...
- [ ] **Long `if x is T { return }` chains exhaust typechecker narrowing.** Past ~80 sequential variant checks the residual narrowed type of `x` collapses to a single arbitrary variant ; the next `if x is OtherVariant` is then flagged T3040 "can never match — `OtherVariant` is never a value of `OneSurvivingVariant`", even though many variants legitimately remain. Hit while writing `vader/bytecode/text.vader:write_op_line` (130+ Op variants). Workaround : convert to a single exhaustive `match v { is X -> ... }` which has no narrowing-precision limit. Likely root cause is `src/typecheck/passes/narrow.ts`'s residual-type bookkeeping — investigate whether it has a hard cap or accumulates float-point precision loss across many `subtract(variant)` operations. Repro on `vader/bytecode/text.vader` history (commit before the match-conversion) or any fresh > 80-arm `if x is T { return }` chain.

- [ ] **Short-form `fn(...) -> R = match <expr> { ... }` rejects a field-access scrutinee.** The expression-bodied fn form accepts a bare ident in the match scrutinee (`fn(td) -> bool = match td { ... }` works in `vader/vm/parser.vader:struct_name_matches`) but errors with `P1003 expected an expression: got =` when the scrutinee is `c.target` or any non-ident expression. Workaround : block-form (`fn() -> R { return match scrutinee { ... } }`) or rebind to a local ident first (`x :: c.target ; match x { ... }`). The block form pattern works fine ; only the short-form expression body trips. Parser likely fails to disambiguate "fn body" vs "fn type" when `match`-after-`=` is followed by a dotted operand. Touch `src/parser/passes/control.ts`'s lambda-body parser.
