...
- [ ] **`std/iter` short-circuiting combinators** (`any` / `all` / `find` / `find_map`). Today's `stdlib/std/iter.vader` ships only the non-short-circuiting family (`map` / `filter` / `fold` / `take` / `skip` / `sum` / `count` / `collect` / `zip` / `chain` / `enumerate` / `flat_map`). Predicate-search shapes like `arr.any(p)` / `arr.all(p)` / `arr.find(p) -> T | null` recur every time a loop "stop on first match" is what the caller wants ; today the only options are a hand-rolled `for ... if return true` loop (cf. `vader/vm/exec.vader:union_matches`, `vader/parser/parser.vader:91-102 MatchResult`, every `arr.contains(x)`-style call site) or `fold` (loses early-exit). Surface area is small : 4 fns, each ~5 lines of body, both eager (`T[]`) and lazy (`Iterator(T)`) flavours. Land alongside the next sprint that touches `std/iter` ; the savings compound across the codebase once §1.13e narrowing fixes also let `arr.find(p)` return a properly-narrowed `T | null` to callers.
...
- [ ] **Reject mutation of constant binding (`x :: ...; x += ...`)** — Vader silently accepts reassignment / compound-assignment / `++` / `--` on a binding declared with `::`. The binding kind (const `::` vs mutable `:=`) is already tracked at the resolver / scope level — it just isn't consulted at the assignment site. Two real bugs discovered during the self-host VM port (Sprint 3 /simplify) :
  - `stdlib/std/string.vader:210` — `index_of`'s `i :: from` then `i += width` ; loop semantics depended on mutation, fixed by switching to `:=`.
  - `vader/vm/parser.vader:254` — `defer_or_resolve_branch`'s `i :: i32(sp) - 1` then `i -= 1`.
...
- [ ] **AST node id boilerplate — auto-assign or derive** — 30+ structs in `vader/parser/ast.vader` repeat `id: usize = UNASSIGNED_NODE_ID` verbatim. Three possible shapes :
  ...
  - (b) **Mixin struct composition** — `BaseAstNode :: struct { span: Span, id: usize = ... }` included as a field in each AST struct ; the syntax `node.id` works through transparent dotting. Requires no compiler work today but changes the AST shape.
  - (c) **Auto-assign at struct-literal construction** when a default field is present — already the case for `id: usize = UNASSIGNED_NODE_ID`, but the field still has to be declared each time. A `@auto_id` decorator on the struct that registers it as "id-bearing" would close the loop.
  (a) is the most reusable. The TS side (`src/parser/ast.ts`) has the same redundancy and could adopt the same pattern. Item is structural ; sequence it before the typecheck port so all AST structs land with their id-bearing decoration uniformly.
...
- [ ] **Type-alias declaration order relaxed (resolver pre-pass)** — `vader/parser/ast.vader:3-7` :
  > the resolver requires every `::` union alias to be declared *before* the struct that references it, so all aliases live at the top of the file and the concrete structs follow.

  Fns and structs already enjoy forward-declaration via the multi-pass typechecker (Pass 1 declare → Pass 2 check bodies). Type aliases should follow the same shape : a dedicated pre-pass in the resolver that registers every alias's *name* before any decl's *types* are walked. Touches `src/resolver/collect.ts` (add an aliases-first sub-pass) — should be ≤ 50 lines. Cosmetic for the user but removes a class of "rearrange your decls" friction the moment the AST gets non-trivial.

- [ ] **`if let` / nullable-bound matchresult cleanup** — Vader has `Token | null` returns but no `if let x := opt { ... }`, so `vader/parser/parser.vader:91-102` invented a `MatchResult :: struct { matched: bool, token: Token }` wrapper, and uses `if p.match_kind(.X).matched { ... }` in 30+ call sites. With the flow-narrowing item above this becomes `if (t := p.match_kind(.X)) != null { ... }` ; full `if let` is the icing. Sequence : **after** the flow-narrowing item — confirm whether 90% of `.matched`-style uses are erased by narrowing alone, then decide if `if let` carries its weight.
...
- [ ] **StringBuilder loses one fragment at the array-doubling step around N ≈ 65 536** (surfaced 2026-05-14 by `bench/string_builder.vader`). Repro : append a short string ~80 000 times to a builder, finalise with `to_string()`. Native binary output is exactly one fragment shorter than expected ; VM run produces the correct count. Same bug under the for-in-range counter loop AND a hand-written `for i < N` counter, so it's not a Phase-A lowering regression. Likely root cause : the C-emit's `vader_array_push` path for `parts` doesn't re-resolve the array's forward pointer through a GC cycle that fires *between* the prior `push`'s buffer-grow and the next `push`'s slot write — the slot is written into the from-space buf which then gets reclaimed before the next iter restarts the bookkeeping. Confirmed pre-existing because reproduces with hand-written counter loop. Bench currently sized at N = 50 000 to stay under the trigger. Fix path : inspect the generated C for the `parts.push` hot path and verify the array-ref is re-loaded post-allocation, same as `vader_array_get` / `vader_array_set` were patched in commit `6a62da1`.
- [~] **GC robustness under high parse-time string-allocation pressure**. Partial fix landed 2026-05-14 ; one residual bug remains on `json_basics`-scale inputs.
  - **Root cause #1 — missing write barriers in Cheney scan (FIXED)**. `vader_gc_scan_box` and `vader_gc_scan_raw` write forwarded pointers into slots without invoking the card-table write barrier. When the slot lives in an old-gen object and the new ref ends up in young, the card stays clean — so the next minor's `scan_old_dirty_cards` skips that object, and the buf pointer becomes stale after the next swap. Compounding this, `vader_major_collect` cleared the whole card table at end-of-cycle, wiping any pre-existing dirty bits for genuine user mutations that hadn't fired yet. Fix : (i) add the barrier in both scan helpers (only when the slot is in old and the target is in young), (ii) replace major's `memset(table, 0)` with `memset(table, 1)` so we lose no edges. Repro before fix : `VADER_GC_STRESS=1 vader run tests/snippets/intrinsic_assert/bytecode.snapshot.virt` segfaulted on `vader_string_byte_at(ptr=0x30, len=1)`. Passes after.
  - **Robustness — `vader_array_get` / `vader_array_set` now resolve forward pointers** before the slot access, matching `vader_array_push`'s existing discipline. Without it, a runtime helper that allocates between two calls (or a callee that triggers GC) can leave the caller with a stale `vader_array_t*` and the next get/set reads/writes the wrong arena half.
  - **Root cause #2 — open**. `json_basics` (3800+ lines, the full self-host workload) still segfaults under `VADER_GC_STRESS=1` despite the above. Trace shows `vader_array_push` being called with 8 consecutive corrupt `vader_box_string` boxes whose `payload.s.ptr` values are `0xb..0x12` (= 11..18, exactly the indices of the `.virt` `string N "…"` declarations that go through `decode_escapes`' builder path). All call sites in the emitted C use `vader_box_string(1u, …)` so it's not a wrong-tag construction ; the pattern points to a `vader_string_t` local in or below `decode_escapes` whose `ptr` field gets overwritten by an iteration counter while held only on the C stack across an alloc. Two leads to chase next : (a) the implicit promotion of a young object containing the `Yielded<string>.f_value` field — `vader_string_t` fields stored inline (not in a box) aren't enumerated in `vader_type_X_ptrs`, so the C-emit doesn't track them ; (b) the conservative C-stack scan in `vader_string_gc_collect` may not cover spill slots produced by the `-O0` compile for fat-ptrs split across argument-passing registers.

  Tooling landed (2026-05-14) : `VADER_GC_STRESS=1` env var — turns intermittent rooting bugs into deterministic failures. Off by default (100×–10000× slowdown).
...
- [ ] **Iterator-chain fusion at the lowerer** (surfaced 2026-05-14 by the `iter_chain` bench — Vader native 27 ms vs Go 3.9 ms = **7× slower**, the worst gap in the bench suite). Pattern : `for x in MapIterator { source: FilterIterator { source: <RangeExpr or array>, pred: F }, f: G } { body }` lowers today to three nested vtable-dispatched `next()` calls per iteration, each allocating a `Done | Yielded(T)` box on the heap. Plan : extend `src/lower/passes/for-in.ts:lowerForIn` with a second special path — when the iterable is a `StructLitExpr` typed as a known `std/iter` lazy-iterator (MapIterator / FilterIterator / TakeIterator / SkipIterator) whose `source` field is itself a known iterable, fuse the entire chain into a single counter loop. The deepest source has to bottom out in a Phase-A-eligible `RangeExpr` or array literal. Conditions for soundness : (a) every layer's `source` is given inline (struct lit, not a variable — variables could be re-assigned), (b) the handler fns (`pred`, `f`) are pure references with no captured state. Lowering target for `for x in Map(Filter(0..<N, pred), f) { body }` :
    ...
    Zero allocations, zero virtual dispatch, two inlinable function calls. Same pattern Rust does for iterator chains. Estimated effort : 2-3 days (detect the layered struct pattern, recursively fuse, route through Phase A's counter-loop emitter, handle break / continue across the synthetic levels). Estimated gain : 5-8× on `iter_chain` ; Vader native should land near Go's direct-loop number.
- [ ] **Inline representation for small tagged unions** (deferred 2026-05-14, scoped out of the bench-driven perf push). Today every variant of `Done | Yielded(T)`, `Some(T) | None`, `Ok(T) | Err(E)`-style unions allocates a backing struct on the heap then boxes it into a `vader_box_t`. For unions whose variants are all empty or one-primitive-field structs (`T` ≤ 8 bytes), the variant fits entirely in `vader_box_t.payload` — the heap alloc is pure waste. Plan : (a) at typecheck, classify each `UnionType` with a derived `unionRep: "inline" | "boxed"` flag (all variants empty-or-single-primitive ⇒ inline) ; (b) extend BcStruct with an `inlineVariant` marker ; (c) c-emit's `emitStructNew` / `emitStructGet` skip `vader_gc_alloc` for inline-variant structs and read/write `box.payload.i` (or `.f`/`.s`) directly ; (d) symmetric VM changes in `src/vm/exec.ts` and `vader/vm/exec.vader`. Acceptance : same semantics for `is Variant` (tag is unchanged) ; `vader_gc_alloc` calls in mandelbrot-style iter chains drop to zero. **Why deferred** : Phase A (for-over-integer-range as counter loop, commit `1e268fd3`) bypassed the dominant Yielded/Done allocation site entirely ; the new `iter_chain` bench confirms the cost is the chain pattern itself (Bun's generators and Java's Stream API land in the same zone), better attacked by **iterator-chain fusion** above. Comes back into scope when chain fusion can't fire (e.g. iterators built up via variables or coming through fn arguments). Estimated effort : 1.5-2 days for the full pipeline change (typecheck + bytecode + c-emit + two VMs + snapshot regen).
