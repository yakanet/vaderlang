...
- [ ] **Cross-module folder modules** — `import "./mod_a"` where `mod_a/mod_a.vader` is the only file inside should land in `project.modules` under id `mod_a` (TS convention), not `mod_a/mod_a.vader`. `tests/snippets/struct_name_collision/` fails today because the entry section of the dump has the import but no `## mod_a` / `## mod_b` section follows ; both modules' Box symbols never reach `external_types`, so `make_a()` types as `fn() -> ?`. Investigate `vader/resolver/loader.vader::module_id_from_root` + `vader/typecheck/dump.vader::dump_program_with_others`.
- [ ] **Generic trait method substitution** — `it.next()` on `it: Iterator(i32)` returns `Yield(?)` instead of `Yield(i32)`. The trait's `next :: fn(self) -> Yield(T) | null` needs `T → i32` substitution at dispatch time, the same way `substitute_self` swaps `Self → for_ty`. Blocks `iter_coerce_array`, `iter_combinators`, `iter_defaults`, `iter_zip_chain`, `for_in_iter_trait`, `custom_iter`, `custom_iter_generic`, `trait_dispatch_generic_iter`, `trait_box_range_iter`.
- [ ] **Generic fn-call argument inference back-propagation** — `nums.reduce(0, (a, b) -> a + b)` where `reduce :: fn[T, U](arr: T[], init: U, f: fn(T, U) -> U) -> U` ; `T=i32` is bound from `nums`, but `U` doesn't bind from `init=0` early enough for the lambda's `expected_fn` to carry `fn(i32, i32) -> i32`. Lambda then types as `fn(i32, i32) -> U`. Iter combinators rely on this.
...
- [ ] **Cross-module type alias unions** — `Pet :: Cat | Dog` in `./types` consumed via `import "./types" { Pet }` should land as `Cat | Dog` in entry's `alias_types` ; today entry's `external_types` carries the alias's Symbol but `type_from_symbol` can't expand it because `alias_types` isn't merged from foreign modules. Blocks `alias_union_in_array`, `namespace_alias_dedupe`.
- [ ] **Empty-array backwidth from first use** — `arr := []` then `arr.push(1)` should retroactively type `arr` as `i32[]`. TS defers the let-binding's type until first concrete usage. Self-host pins it to `?[]` immediately. Blocks `array_push`.
