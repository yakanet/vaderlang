...
- [x] **Auto-coerce `T[]` → `Iterator(T)`** — done. Typechecker accepts the assignability (`isAssignable` in `src/typecheck/types.ts` matches `Array(T)` against `Trait{Iterator}(T)`) and records each coercion site in `arrayIterCoercions: Map<A.Expr, Type>` (call args, `let` with explicit type, `return` slots — `src/typecheck/passes/call.ts`, `stmt.ts`). Lowerer reads the side-table at the top of `lowerExpr` (`src/lower/passes/expr.ts`) and wraps via the shared `wrapArrayAsIter` helper extracted from for-in. Comptime engine pre-registers each coerced `ArrayIterator(T)` instance so mono materialises the `next` impl. Test: `tests/snippets/iter_coerce_array/`.
...
- [ ] **Hash-cons `Type` values + re-key `implMethodEntries` by reference identity** (Phase B of the mono-identity cleanup, 2026-05-11). Today `MonoProject.implMethodEntries: Map<FnDecl, Map<string, MonoEntry>>` uses `displayType(arg).join(",")` as the inner key — fragile (sensitive to `displayType` rewrites), allocatif (one string per lookup), and not structurally unique (two `equalsType` types with divergent `displayType` outputs would key as distinct entries). Same string-keyed shape leaks into `fnInstanceEntries`. Phase A (commit `6b6905c`) split `MonoEntry.id: number` off the mangled string so the foundation is in place ; this is the follow-on. Plan : (1) `Type` constructors funnel through a global builder cache that returns the same JS object for structurally-equal types — `unionOf` already does half of this for unions, generalise to Struct / Trait / Array / Tuple / Fn. Primitives are already canonical (`TY.i32` etc. are singletons). (2) Once every Type has a stable JS identity, `Map<readonly Type[], MonoEntry>` is viable via tuple-keying or a small trie (one Map level per arg). (3) Drop `displayType(...).join(",")` calls in `specialize.ts`. (4) Verify no consumer reads the raw key string (search for `argsKey`-shaped lookups). Touches : `src/typecheck/types.ts` (interning), `src/comptime/specialize.ts` (argsKey replacement), `src/lower/passes/{expr,for-in,display-coerce}.ts` (`lookupImplEntry` callers). Effort : ~½–1 day, mostly mechanical once the interning layer lands. Side-benefit : the existing `equalsType` walks become `===` reference checks, faster typecheck on large impl registries.
