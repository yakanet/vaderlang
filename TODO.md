...
- [ ] **`.virt` reader silently drops unknown opcodes** (noted 2026-07-27). `virt_reader::try_push_op` returns `false` for any line it can't parse and the caller `continue`s, so a dump carrying an instruction the reader doesn't know runs with that op MISSING — unbalanced stack, plausible wrong output, no diagnostic. Found by removing an opcode while a stale `.virt` still used it: `path_basics` printed empty filenames rather than failing, and a subsequent `UPDATE_SNAPSHOTS=1` overwrote nine correct `vm.snapshot` oracles with the wrong output. Fix direction: distinguish a non-instruction line (what the silent drop is actually for) from an unrecognised opcode, and trap on the latter. Cross-ref the snapshot-regeneration order trap — regenerate `bytecode.snapshot.virt` BEFORE the vm oracles, or `vader_vm.test.ts` measures the old dump.
