...
- [ ] **GC robustness under high parse-time string-allocation pressure**. Surfaced three times on the self-host VM port :
  - Sprint 13 (2026-05-14) — `is_wide_int_op` allocated 4 interpolated strings (`"i64.${verb}"`, …) per call, ~4 × per .virt op-line × ~3800 lines on json_basics. Cascading GC pressure corrupted `body.ops` reads (instr.tag came back as `I64Val` mid-dispatch). Worked around by switching the helper to a one-shot slice + char-by-char compare.
  - Tokenizer attempt (2026-05-14) — refactored `parse_op_line` to a head/tail dispatch via `tokenize_op_line` (line.split + trim per token). Same json_basics symptom : `"name"` key in the parsed JSON came back as `"vers"` (first 4 chars of `"version"`, suggesting buffer reuse post-evacuation). Reverted ; only the `try_push_op` collapse landed (-99 lines).
  - Stress-mode reproducer (2026-05-14) — `VADER_GC_STRESS=1` (forces a minor collect on every `vader_gc_alloc`) makes the bug deterministic on **any** snippet, even a 6-line `.virt` like `tests/snippets/intrinsic_assert/bytecode.snapshot.virt`. Crash signature : `vader_string_byte_at` receives `(ptr=0x30, len=1)` on entry to `strip_comment`. Stack trace : parse_virt → ArrayIterator.next → corruption appears at `((Yielded<string>*)box.payload.obj)->f_value` read. Disabling the string sweep entirely does **not** fix it, ruling out "buffer freed, fat-ptr now stale". The corruption is of the `vader_string_t` value itself, not the underlying buffer.
  
  Root-cause hypothesis (not yet confirmed) : the C-emit's `gc_roots[]` lists only `vader_box_t*` locals — `vader_string_t` locals and function parameters (e.g. parse_virt's `l0 source`) are excluded. They rely on the conservative C-stack scan in `vader_string_gc_collect` (step 5 : setjmp register dump + scan from `__builtin_frame_address(0)` upward to `g_stack_bottom`) to keep their buffers marked. Two suspicions : (i) under `-O0` the scan window may not cover compiler-generated spill slots for the fat-ptr split across argument-passing registers ; (ii) the corruption pattern `0x30` is reproducibly a small integer, suggesting a stack-slot reuse or a write into a `vader_string_t` storage location by code that thought it was something else (header field overlap ?). Need to dump stack frame addresses + actual reg/spill state at the corruption point to nail it.

  Fix candidates : (a) extend the C-emit to add `vader_string_t` locals/params to a separate "str_roots" list and have the string GC scan them precisely instead of relying on conservative ; (b) make the C-emit force every `vader_string_t` into a stack-resident storage with `volatile`/`__attribute__((used))` so the conservative scan reliably catches it ; (c) at the runtime side, scan `vader_string_t` fields *of every Cheney-heap object* by emitting their offsets in a new `str_offsets` array of `vader_type_info_t` and walking them at mark time — orthogonal to (a) but covers in-heap string fields too (e.g. `Yielded<string>.f_value`).

  Tooling landed (2026-05-14) : `VADER_GC_STRESS=1` env var in `runtime/c/vader_runtime.c` — turns intermittent rooting bugs into deterministic failures. Off by default (100×–10000× slowdown). Useful for any future class of GC-rooting investigation.
