...
- [ ] Snapshot-test parity : **109 / 111** today (2026-05-07). Remaining two skips (`std_string`, `std_string_builder`) hit a CJK/multi-byte UTF-8 limitation in `std/string.char_at` (byte-indexed instead of codepoint-indexed) — stdlib bug, not a parser-port bug. Tracked as a separate item in §1.13.
...
- [x] **Tuple types and destructuring**. Bracketed type form `[T1, T2, ...]` (≥ 2 elements) introduces heterogeneous tuples ; the array form moved to postfix `T[]`. Disambiguation of seq literals is contextual (TS-style) : `[1, 2]` is an array, `[1, "x"]` is a tuple, annotations win. Field access via `t.0`, `t.1`. Destructuring works in `let` (`[a, b] := pair()` ; nested + wildcards supported) and in match arms (`[a, b] -> ...`). Tuples lower to anonymous structs with synthetic fields `_0`, `_1`, ... — no new BC ops or runtime types. Hard cut on the legacy `[T]` form (P1024). Follow-up : update the Vader self-host parser at `vader/parser/*.vader` to mirror the new AST node shape (LetBinding, SeqLitExpr, TupleTypeExpr, TuplePattern) so `parser_parity` tests can re-enable.
...
- [ ] **Implicit selector expressions** (Odin-inspired) : when the expected type is a known enum / tagged union variant set, allow `.Variant` as shorthand for `TypeName.Variant`. Examples : `f = .A` when `f: Foo` is an enum ; `match x { is .Some(v) -> ... }` ; `[Day.Monday, .Tuesday, .Friday]` inside an array literal of `[Day]`. Triggers only where a target type is supplied by context (assignment, fn arg, return, match arm, struct-field init). Touches : parser (allow leading-`.` expression head), typecheck (resolve via expected-type), resolver (no global lookup when the leading `.` is present). Cf. SPEC §8 *Enums* / §9 *Unions*.
...
- [ ] **`@assert(condition)` — compile-time assertion**. Top-level or in-comptime statement that fails the build if the condition (a comptime-evaluable boolean) is false. Useful for invariants over `size_of(T)`, alignment, type predicates inside generics, build-config sanity. Reuses the existing comptime VM ; one new decorator-statement form in the parser. Diagnostic includes the source span and, when available, the value of each side of a comparison.
- [ ] **`@partial` on `match`** : opt-out of exhaustiveness checking for a single `match` expression / statement when the user intentionally handles only a subset of variants. Default stays exhaustive (today's behaviour). `@partial match x { is .A -> ... }` does not require a wildcard arm and emits no missing-case diagnostics. Prevents devs from working around exhaustiveness with bogus `_ -> {}` arms that silently swallow new variants. Touches : parser (recognise `@partial` as a match modifier), typecheck (skip the exhaustiveness pass when the flag is set). Cf. SPEC §13 *Pattern matching*.
- [ ] **`@deprecated(message)` decorator** on fns / consts / structs / traits / variants. Emits a warning at every reference site with the supplied message ; reference still compiles. Optional second arg `since: "1.2"`. Touches : decorator parser (already supports `@name(args)`), resolver (annotate decl), typecheck (emit a warning per resolved reference, deduplicated per source span). Trivial against the existing decorator pipeline ; ~50 lines. Useful for stdlib evolution post-1.0.
