...
- [ ] **`for x in self` over an abstract `Iterator<T>` types the loop var `?`, not `T`** (found 2026-07-24). `for_in_element` (`vader/typecheck/stmt.vader:1333`) handles arrays and structs but has no `TraitType` arm, so inside a generic combinator's own body — `my_map :: fn<T, U>(self: Iterator<T>, …) { for x in self { … } }` — every loop variable records as `?` in `expr_types` (visible in `tests/snippets/fuse_generator_chain/typecheck.snapshot`, and in LSP hover). **The one-line fix is NOT enough**: returning `tr.args[0]` for `Iterator<T>` types the var `T` correctly and self-compiles, but breaks generator materialisation in std/iter — `midir/emit: no field 'value' on Yield<T>` in `__gen_state_*$Iterator$next`, with `erased_iter_combinator` / `iter_combinators` / `iter_zip_chain` failing and `iter_zip_chain`'s bytecode snapshot collapsing by ~1300 lines. Something downstream relies on the loop var being unresolved to keep the erased generator path; typing it `T` sends it down a substitution that never completes. So the real work is in the generator-state materialisation (`Yield<T>`'s type arg under erasure), with the typecheck arm landing after. Attempted and reverted in full; the tree is unchanged. Not a correctness bug today — an erased body and an unresolved element agree on the boxed representation — but it is a type that lies, the same shape that produced the erased-vs-packed family (see `docs/HISTORY.md` §3.8).
...
- [ ] **The `.virt` directive parsers still degrade quietly** (noted 2026-07-27, found by the review pass on the line-level fix above). Each one drops or substitutes on malformed input and returns as if it had succeeded, so the pool it feeds comes back short or wrong with nothing said. `parse_type_decl_bc` (`vader/vm/virt_reader_bc.vader`) falls through to `bc.BcRef { .trait_name = "" }` for any `type N <kind>` it doesn't recognise — the exact `BcStruct`→`BcRef` placeholder shape that was blocker #1 of the self-host bytecode audit, where an opaque ref makes every downstream field access a miss. `parse_struct_type_bc` skips a field whose `name:tid` doesn't split or whose tid doesn't parse, shifting every later field index — the struct-field analogue of the pc shift just fixed. `parse_int_list` (`virt_reader.vader`) documents "malformed entries are skipped silently" and backs both union variant lists and fn param lists. `parse_import_decl_bc` / `parse_impl_decl_bc` / `parse_vtable_decl_bc` / `parse_export_decl_bc` / `parse_data_decl_bc` each `return` mute on a malformed line, dropping a whole pool entry and shifting every index after it. Same fix shape as the line level — these are internal, so they can return `VirtParseError` directly or take an out-param; the line number is already in the caller's hand. Worth doing before anything relies on `.virt` as an interchange format rather than a debug dump.
- [ ] **A distinct-type cast loses its type when a method is chained onto it** (found 2026-07-27 converting the CLI's paths to `std/path::Path`, a newtype over `string`). Writing `Path(string(s).replace_chars_where(…))` — cast the newtype down to `string`, call a `string` method on the result — passes the typechecker and then **panics in midir**: `no field 'replace_chars_where' on Path in 'std_path$to_posix' — a method reference on a primitive isn't a first-class value`. So the cast's result still carries the newtype at lowering, and the method lookup runs against `Path` instead of `string`. Binding the cast first (`base :: string(s)` then `base.replace_chars_where(…)`) is the workaround, untested but the obvious next probe. Two things make this worse than it reads: the panic names a function the author didn't touch, and it is only reachable through `vader test <module>` / the per-module isolated compile (`tests/vader_e2e.test.ts` runs one per `stdlib/std/*`), so a plain `vader build` of a program using the same code can pass while CI fails. Reduce it — a two-line newtype plus one chained method should be enough — before deciding whether the fix belongs in the cast's lowering or in the method-resolution walk.
...
- [ ] **`push_all` segfaults in one place, and the reduction doesn't reproduce** (found 2026-07-27 wiring the declarative CLI). `vader/cli/spec.vader::command_specs` written as `all.push_all(cmd.flags)` makes `vader lsp --stdlib-root=…` die with **SIGSEGV (exit 139), no diagnostic, no output**; written as a `for f in cmd.flags { all.push(f) }` loop it exits cleanly. Reproduced twice in both directions, so the call IS the trigger. The same `push_all` for the `build` action — whose `.flags` is a const of the same module — is fine, and `vader help lsp` (which reads the same array through the renderer, not `push_all`) is fine too. **What is NOT the cause:** a reduced snippet doing `push_all` on a module const from another module, *and* on an array reached through a local const's struct field that was initialised from a foreign const, prints correct results and exits 0 — both eliminated. So the cross-module hop alone isn't it; the real trigger needs something else the CLI has (three modules deep — `CommandSpec` from `std/cli`, `COMMANDS` from `vader/cli`, `LSP_FLAGS` from `vader/lsp`; a struct with defaulted fields; a `ValueSpec[] = []` default) and the reduction hasn't found it yet. Next steps: bisect by moving `LSP_FLAGS` into `vader/cli` (does it survive?), then by dropping the field defaults on `FlagSpec`. `push_all` lowers to the element-agnostic array intrinsic (`stdlib/std/core/array.vader:28`), so the suspicion is an erased-vs-packed element mismatch — the same family as `docs/HISTORY.md` §3.8. **Do not "simplify" the loop in `command_specs` back to `push_all`** until this is understood; the comment there says so.
- [ ] **A single-dash CLI token is swallowed instead of rejected** (noted 2026-07-27). `std/cli` has no short flags at all — `FlagSpec` carries one `name` and no alias field, and `parse_inner` (`stdlib/std/cli/cli.vader:82`) pushes anything not starting with `--` straight to `positionals` without ever looking for a spec, in the strict `parse` mode as much as in `parse_known`. A `-o` is therefore indistinguishable from a filename before any caller sees it; `first_file_arg` (`vader/cli/main.vader`) then skips it as a non-first positional. So `vader build x.virt --target=c -o out` exits 0 having written `x.c` next to the input — no effect, no diagnostic — while `--out /x` (space form) correctly errors with `--out: missing value, expected --out=<value>` and `--help` errors with `unknown flag --help`. The parser only complains about flags it recognised; a single dash never reaches that gate. (Found by writing `-o` out of habit and littering 384 stray `.c` files across the snippet corpus, which then broke `parity-cemit` with duplicate `_main` symbols.) The module header scopes short flags out explicitly ("Add when a real caller needs them"), so the question is which of two things we want: **(a)** the minimum — treat an unrecognised `-x` as an error instead of a positional, no aliases introduced; or **(b)** real short aliases, which means an `alias` field on `FlagSpec` and a decision on whether `-abc` bundling and `-o value` (space form) come with it. (a) alone closes the silence.
