...
- [ ] **Extend destination-passing to short-circuit `&&` / `||`** (surfaced 2026-07-25, `/simplify` on the dest-passing generalization). `build_short_circuit` (`vader/midir/build.vader`) is the third result-slot builder — it declares `and_res` / `or_res` written across blocks and read after the join, exactly the phi-slot class `build_if` coalesces — but it is NOT in the dest-passing set (excluded only because it is a `LoweredBinary`). So `let x = a && b` and an `if` arm `{ a && b }` still build `and_res` then *move* it into the destination. Fix (symmetric to `build_if`): give `build_short_circuit` a `dest: LocalId | null` param (`result :: dest ?? declare_local(result_name, e.type, null)`), add a `LoweredBinary`-And/Or arm to `build_expr_into`, and admit it in the `LoweredLet` gate. Collapses one slot + move per short-circuit in destination position (not an `a && b && c` chain internally — each lhs is a real branch cond). When done, fold the two enumerations of the dest-passing shape-set (the `LoweredLet` gate + `build_expr_into`'s dispatch) behind one `is_dest_passing_shape` predicate so they can't drift.
