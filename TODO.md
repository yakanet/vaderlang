...
- [ ] **Remove the `T!` error-union sugar** (added 2026-06-01). `T!` desugars to `T | Error` (the existential `Error` trait). It reads cleanly when the only error is the generic one, but becomes confusing the moment a fn returns a *concrete* error — e.g. `parse_int` now returns `i32 | ParseError`, where the caller matches `is ParseError`, not `is Error`, so `T!` and `T | ConcreteError` look interchangeable but aren't. Drop the `!` sugar; write explicit `T | <Error>` everywhere. Migration: every `-> X!` decl → `-> X | <ConcreteError>`; every `is Error` match catching such a result → `is <ConcreteError>`; **consolidate the duplicated error structs** — `std/string`, `std/numbers`, `std/semver` each define their own `ParseError`; pick one (in `std/string`) and import it (no module aliasing in MVP makes the name-clash a hard blocker until the dups are removed — this already broke semver's `is Error` matches when `parse_int` went concrete). The `?` operator must keep propagating any value implementing `Error` (verified working on the self-host VM with concrete unions). Note: the self-host VM also can't dispatch the existential `is Error` + virtual `.message()` on a Vader error struct (impl-table/vtable not materialised for an existentially-boxed stdlib impl — see the self-host-VM-stdlib-impl-materialization memory note), so concrete unions are the VM-friendly form regardless.
...
- [ ] **`string.bytes()` as a zero-copy read-only intrinsic** (added 2026-06-01). Make `bytes()` an `@intrinsic` returning an array whose pointer aliases the string's own interned memory (no copy), instead of today's O(n) copy. Then most of `std/string` collapses onto array ops: `byte_len(s)` → `s.bytes().len`, `byte_at(s, i)` → `s.bytes()[i]`, and `byte_find` / `bytes_match_at` / `split` / `trim` (added 2026-06 in the pure-Vader port) reimplement via the generic array/iterator machinery in `std/iter`. **Blocked on a read-only / immutable array notation** — none exists yet; the returned view must forbid writes back into the interned (shared) string. Relates to the A7 `T[]` view-aliasing model.
...
- [x] **Resolver port follow-ups** (closed 2026-05-26, parity-diagnostics 1181 pass / 0 skip / 0 fail). Closed items :
