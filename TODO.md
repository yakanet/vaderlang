...
- [ ] **Finish destination-passing in `build_if` → retire `else_chain_if`** (follow-up to the 2026-07-25 match-lowering density optim). `build_if` (`vader/midir/build.vader`) gained a `dest: LocalId | null` param so a value-`match` / `else if` chain coalesces its per-arm `if_res` slots onto one — but `dest` is only self-wired to the recursive else-chain, gated on the structural `else_chain_if` shape (`{ stmts: [], trailing: LoweredIf }`). The robust, shape-independent form threads `dest` through the other slot-materialising sites — `LoweredLet` (`build.vader` ~409-417 : `build_expr` + `InstrMove dst <- value`, which a `dest`-aware build would elide for `let x = if…{}`) and `LoweredReturn` — so a general `build_expr_into(dest)` covers all value expressions and the `else_chain_if` structural match dissolves. Not required (the peephole works today) ; do it if a second consumer of dest-passing appears.
