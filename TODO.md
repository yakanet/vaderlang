...
- [ ] **Namespace-imported aliases don't unify with direct imports** (surfaced 2026-05-14 while porting `vader/lsp/ast_tokens.vader`). `import "vader/parser/ast" as AST` exposes every alias as `AST.X` ; the SAME alias surfaced via `import "vader/parser/ast" { X }` (or via a transitive return-type from another module) ends up as a distinct `X`. Concretely : the walker's `walk_decl(d: AST.Decl)` couldn't accept `parse_source(src).program.decls : Decl[]`, with T3001 reporting `expected ... | ImportDecl | ... got ... | ImportDecl | ...` — same union spelled-for-spelled. Reverse direction (`is AST.ImportDecl` in a match arm whose scrutinee is `Decl`) trips T3040 ("test can never match"). Workaround : import every name explicitly via `import "vader/parser/ast" { X, Y, Z, ... }`, which is verbose but unifies cleanly with the parser's own return types. Touches `src/resolver/resolve.ts` (namespace-import binding) + `src/typecheck/types.ts` (type-identity comparator) — the alias's underlying union should compare structurally (or by definition site) regardless of which alias name surfaces it.
