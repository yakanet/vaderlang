...
- [ ] **Adopt `s[i]` over `s.char_at(i)`.** `string implements Index(usize, char)` is wired in `std/core.vader:515` (host-provided as `std_core$string$Index$at` in `src/vm/host.ts`), so `s[i]` is the idiomatic surface for codepoint access. Sweep `stdlib/`, `vader/`, the formatter, parser, lexer — replace `.char_at(i)` call sites with the bracket form where it reads cleaner. Keep `.byte_at(i)` in hot byte-level scanners (`vader/fmt/comments.scan_comments`, lexer body) since `[i]` is char-typed and forces UTF-8 decode per step.
...
- [ ] **`std/cli` (or similar) — command-line argument parsing library**. Motivated by the Vader CLI itself (today `vader/cli/main.vader` hand-rolls argv tokenisation, which is fine for the current `fmt` subcommand but won't scale to `run` / `build` / `test` / global flags). API to be designed — open questions : declarative spec vs. builder ; subcommands as first-class ; how to surface `-h` / `--help` ; required vs. optional vs. multi-valued flags ; positional args ; how errors flow back (`Result`-style vs. printed-and-exited) ; whether the lib owns `--version` / `--help` rendering or just the parse step. First user once it lands : rewrite `vader/cli/main.vader` on top of it.
...
- [ ] **Range[T] impl deduplication** — `std/core` ships 11 near-identical `Contains` + `Iterator` impl pairs (one per integer width + char). The bodies differ only in the type name. Three reasonable cleanups : (a) bounded generic impl `Range[T: Comparable & Add] implements Iterator[T]` once Vader supports bounds on `implements` blocks plus operator-trait dispatch on bounded type-params and a `T(1)` literal coercion ; (b) a `@for_each_int_type T { ... }` comptime macro ; (c) compiler-magic synthesis when `T` is a primitive numeric. (a) is the cleanest, (c) the cheapest.
