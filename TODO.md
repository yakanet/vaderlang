...
- [ ] **GC robustness under high parse-time string-allocation pressure**. Surfaced twice on the self-host VM port :
  ...
  The .virt parser shouldn't have to dodge `String.split` / interpolation to stay correct. Two candidate fixes : (a) pin / mark heap pointers that the parser holds across allocation points so they survive `vader_gc_forward` even when the source object moves ; (b) pre-allocate a reusable tokens buffer inside `parse_op_line` so the per-line allocation count drops back to ~1. Without one of these, the head/tail tokenizer refactor and the ~−260 LOC it would unlock stays parked.
