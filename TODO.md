...
- [ ] **Typecheck body-walk reduction (lever A) — big refactor, planned** (2026-06-21). Type-checking is the self-compile's #1 cost (~42 %); each non-entry module body is walked **~4×** (settle pass0+pass1 + import-loop throwaway+real) = **82 % of typecheck**, settle alone = **267 MB GC-copied** (biggest churner). Two ultracode adversarial analyses PROVED point-fixes can't cut the walks: each removal reintroduces the `abs`/`i64.lt` return-truncation or breaks stdlib materialisation (GATE B1); idempotent `push_overload` unblocks only the import-loop `clear`. The walks are load-bearing for two structural reasons — (1) non-entry `external_types`/`impl_methods` are wired only INSIDE the import loop, AFTER `settle`, so the walks see different resolution environments; (2) forward-reference return-settling over `decl_types` needs ≥2 passes regardless. Cutting the 4× needs a **structural refactor**: populate the full environment for all modules ONCE up front (via a new lightweight "return-only" walk mode that settles returns + materialises but builds no `expr_types`), then a single full walk per module. Large, high-blast-radius, multi-session. **Full design: [`.claude/plans/typecheck-walk-reduction.md`](.claude/plans/typecheck-walk-reduction.md)** (phases P0 alloc-counter → P1 return-only mode → P2 restructure → P3 measure). Prereq P0 (alloc counter) also de-risks by measuring first. Lever (c) — idempotent `push_overload` + drop clear — lands separately (small cleanup, not a walk cut).
...
- [ ] **Leading `&&` / `||` line-continuation not parsed** (added 2026-06-21, found while landing the string-`+` concat fusion). A boolean condition broken across lines with the operator at the **start** of the continuation line —
  ```
  if b.op == BinaryOp.Add
      && is_string_type(ctx.types.expr_type(b.left))
      && is_string_type(ctx.types.expr_type(b.right)) {
  ```
  — does not parse : `if b.op == BinaryOp.Add` is taken as a complete condition, the parser then expects `{` and reports `P1002 expected `{` to open block (got newline)`, and the next line's `&&` is `P1003 expected an expression: got `&&``. The brace mismatch cascades (the rest of the file fails to parse, a wall of spurious R2006 `unresolved identifier`). Worse, an OLDER compiler (the bootstrap seed/stage0) **error-recovers silently** and lowers a malformed AST → the same `midir/emit: … a method reference on a primitive isn't a first-class value` codegen panic family as the comma-enum bug above (cost ~an hour to trace : the panic pointed at a synthetic `__lambda_NNNN`, not the source `&&`). Workaround : keep the whole condition on one line, or extract a predicate fn with early `return`s (what the concat-fusion `is_primitive_string_concat` does). Fix : the parser must continue an expression when a line ends mid-binary-operator OR the next non-blank token is an infix operator (`&&`, `||`, and arguably `+`/`==`/etc.) — a `\n`-as-continuation rule in `vader/parser/parser.vader`'s expression / statement boundary logic. Until then a leading-operator continuation is a **silent miscompile on old binaries**, so it's worth a clean `P-` diagnostic at minimum. SPEC.md should state the line-continuation rule once decided.
...
- [ ] **Snapshot flip** — `tests/snapshot.test.ts:22` still compares
