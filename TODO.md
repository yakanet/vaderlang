...
- [ ] **Unbox struct field read after `is StructTy` pattern match.** Repro : `tests/snippets/map_set_iter/_main.vader` — the for-in over `MutableMap.iter()` binds `entry: Entry(K, V)` via the pattern path ; reading `entry.value` (typed `i32`) emits a load that the C-emit leaves as `vader_box_t`, so the next op `total + entry.value` compiles as `int32_t + vader_box_t` and the C compiler rejects it. VM works (tag carries the type). `tests/native.test.ts` skips silently when cc fails, so the snippet still passes — but the natif is unusable for any map / set iteration. Surface area : `src/c_emit/` extraction of a struct field whose static type is a primitive after a narrowing pattern. Tracked as a follow-up to §1.5b iterator work (commit `3ac4a14`).
...
- [x] **Iterator impls for `MutableMap` / `MutableSet`** (2026-05-11). `MapIterator(K, V)` walks buckets + chains lazily, yields `Entry(K, V)`. `SetIterator(T)` reimplements the walk directly on the backing `MutableMap(T, bool)` rather than wrapping `MapIterator` — wrapping forced a runtime `match` on the nested generic `Yielded(Entry(T, bool))` payload that the VM's variant tag check doesn't drill into. Both expose a factory `iter()` so `for entry in m.iter()` / `for v in s.iter()` work. Snippet : `tests/snippets/map_set_iter/`. Known gap : C-emit doesn't unbox `entry.value` after the `is Entry(K, V)` narrowing — native build fails with `int32_t + vader_box_t`, `tests/native.test.ts` skips silently. Tracked in §1.9b.
...
- [ ] **`is Struct { field, … }` (struct field destructure) traps at runtime when the scrutinee is a union** (uncovered 2026-05-12 while spot-running SPEC examples). Repro : `match value { is null -> "n" is Point { x, y } -> "(${x}, ${y})" }` on `value: Point | null` raises `vm: reached unreachable` at `is Point { x, y } ->` ; `match value { is null -> "n" is Point as p -> "(${p.x}, ${p.y})" }` works (binding-only form). The existing `tests/snippets/match_struct_pattern_binding/` snippet covers the same destructure shape but on a *non-union* scrutinee, which is why CI doesn't catch it. Likely the lowerer's `IsPattern` recursion into `pattern.inner` (cf. earlier as-binding fix, §1.13c) only fires on the direct path ; the union-cascade lowering needs the same recurse + per-arm cast. Suspected files : `src/lower/passes/match.ts:narrowToTupleVariant` and the StructPattern cousin (or absence thereof). Add a regression `tests/snippets/match_struct_pattern_in_union/`.
...
- [ ] **Enum-to-int direct cast (`i32(t)` / `t.ordinal()` for `enum(IntRepr)`)** — every enum that needs its declaration index pays a full-arm match. See `vader/lsp/semantic_tokens.vader:52-68` (`token_type_int`) and `:76-81` (`modifier_bit`). The compiler already knows the integer value (it's how the enum lowers — cf. §1.17). Two equivalent surfaces : (a) implicit conversion `i32(t)` for `enum(u8)`-and-friends, (b) auto-derived `t.ordinal() -> i32` method on every enum. (a) is more uniform with the existing `i32(c)` cast for chars. Touches `src/typecheck/passes/expr.ts:inferCast` (recognise the enum-to-int form) and the lowerer (already trivial — the value is just the enum's `LoweredIntLit`).
...
- [ ] **Transitive monomorphisation through nested generic calls** — confessed in `stdlib/std/collections.vader:67-76` :
  > Lazy bucket init — inlined here because nested generic calls aren't observed by the comptime InstanceRegistry yet (transitive monomorphization is deferred), so a `private ensure_buckets($K, $V)` helper would not get materialised at the right K/V instantiation.

  At 16 lines of inlining today it's tolerable, but the typechecker (4-5k TS lines, 1000+ generic call sites) becomes unportable without it. Wires through `src/comptime/instances.ts` + `src/comptime/specialize.ts` — the `InstanceRegistry` already walks bodies for direct calls ; the missing piece is recursing into a generic fn's body under each `(fn, type-args)` instantiation. Already alluded to in §1.19 Layer 2 ("monomorphisation as comptime evaluation") but no concrete subtask. Sequence : **before any typecheck port** lands in Vader.
...
- [ ] **Self-host lexer u64 overflow** — `vader/lexer/lexer.vader:decode_int_literal` returns `i64`, so any unsigned literal in `(i64.MAX, u64.MAX]` wraps to negative. The TS lexer keeps a `bigint` through decoding. Symptom : `18446744073700000000` becomes `-9551616` on the self-host path. Surfaces as a parity divergence on `tests/snippets/numeric_context_sensitivity/`, currently skip-listed in `parity.test.ts:UTF8_KNOWN_DIVERGENT` + `parser_parity.test.ts:KNOWN_FAILURES`. Fix : promote the value carrier in `IntLitExpr` (`ast.vader`) to a u64-sized representation, thread through `parse_int_in_base`, the dump pass, and the bytecode emitter. Could double-buffer as `[i64.MAX-sized magnitude, sign]` if a true u128/bigint is too costly. Loses no correctness as long as the lexer reports decode-overflow when the literal exceeds u64.

- [ ] **Flow-narrowing on arbitrary unions — extend `if x is T { … }` and `if x != null` to all variants** — today `src/typecheck/passes/narrow.ts:detectNullCheck` only matches `BinaryExpr(.Eq/.Neq, ident, NullLit)`. Generalise to `BinaryExpr(.is, ident, TypeExpr)` (so `if e is IfExpr { e.then.… }` narrows like `match e { is IfExpr -> … }`), and to **any** variant-removing comparison once unions carry a tag stable across the discrimination. Mirrors what `match` already does inside arm bodies. Also extend the LHS to `FieldExpr` for receiver-narrowing (`if state.cache != null { state.cache.get() }` ; today this forces a `c :: state.cache ; if c != null { c.get() }` rebind). Touches `detectNullCheck` (rename → `detectVariantNarrowing`), and probably the lower-side `(Union) == null` rewrite to handle the general `is`-narrow case. Snippet : `tests/snippets/if_is_narrow/`. Concrete site that motivates the extension : `vader/parser/parser.vader:wrap_as_block` currently uses `match body { is BlockExpr as b -> return b _ -> {} }` then falls through to the wrap-construction ; with `if body is BlockExpr { return body }` narrow, the helper collapses to two lines.
