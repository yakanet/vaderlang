...
- [~] **`StringBuilder.to_string` read-back regression** (added 2026-06-15, found via ABI benches). The Buffer-backed `to_string` (`stdlib/std/string_builder/string_builder.vader`) reads every byte out of the `Buffer` into a fresh `u8[]` via `load_u8` + `push`, then interns that array with `bytes_to_string` — a double copy. Measured **3.3× slower than `main`'s `[string]`-parts + `vader_string_concat_all`** model (`bench/string_builder` : 7.9 ms → 26.6 ms). This path backs **all** `${…}` interpolation, so it taxes every program that prints. Fix : intern the `Buffer`'s `[0, len)` range directly (a thin `Buffer → string` `@intrinsic` reusing the runtime's contiguous-byte intern), dropping the read-back loop. Tracked / being addressed alongside the Target-ABI close-out (the S5 Array flip and StringBuilder share the Buffer primitive).
