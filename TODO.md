...
- [x] **`get` boxed its `V` return with the array-element type-index, not the canonical V index (FIXED 2026-06-20 via the `Box` op)**. A concrete primitive read from a monomorphised `V[]` (e.g. `MutableMap(string, usize)::get` returning `evals[i] : usize` as `V | null`) was boxed at the erased return with the COARSE ValType tag (`i64`, 99) instead of the SEMANTIC type (`usize`, 29), because the C-emit's implicit return-coercion only has the coarse ValType. Result: `rows_by_key[k] is usize` in `build_vtables_from_entries` failed → 4 duplicate `vader_vt_Error__message` C dispatchers → invalid C, no fixed point. lldb root-caused (`build/vader2dbg`, python watchpoints): get found the key but `vader_box_i64(99u,…)` vs caller `tag == 29u`. Chaining never hit it (its get returns a struct FIELD, interned to 29). **FIX = new bytecode op `Box { type_id }`** emitted at `midir/emit.vader::TermReturn` when a concrete primitive flows into an erased ref/union return; c_emit boxes with the precise tag, VM no-op (coarse `I64Val` matches any I64-width `is`). Built without reseed via a 2-stage bridge (chaining+Box → compact+Box); **fixed point reached** (vaderC.c==vaderD.c byte-identical), suite 2283/0, −14% self-compile RSS. Erasure box-vs-raw / type-table dedup family ([[erasure_audit_dotnet_model]]).
  - **Follow-up (deferred): `Box` op is emitted RETURN-ONLY — audit the other erasure boundaries (2026-06-20).** The same coarse-tag / dual-index hazard exists wherever a concrete primitive is widened into a `ref`/`any` slot, not just at `TermReturn`. Audit done this session — the `box_expr` call sites and the type-index each passes:
    - `c_emit/walker.vader:1192` call result → `sig.result_type` (semantic) ✓
    - `c_emit/walker.vader:1475` struct-field store → `sf.type_index` (declared field type) ✓
    - `c_emit/walker.vader:1765/1802/1823/1835` array elem store / push → `elem_tag` / `op.type_id` (element type) ✓
    - `c_emit/walker.vader:1536` `emit_box_op` → `op.type_id` (the `Box` op's carried tag) ✓
    - **`c_emit/body.vader:323` `box_expr_unknown` → `primitive_tag_of(types, val)`** ⚠️ — derives the tag from the COARSE ValType (returns the FIRST `BcPrimitive` of that stack class, i.e. the standalone index). Used by `coerce_expr` for **fn-arg widening into an erased param** and **dispatcher-result wrapping**. Works TODAY *because* `primitive_tag_of` canonicalises to the standalone primitive (the very index the `Box` op re-interns to), but it's fragile: it silently falls back to **tag 0** (a `type_check` that always misses) when the standalone primitive isn't materialised in the table, and it's the same family of bug. Not observed failing — self-compile happens not to pass an array-element-read concrete primitive straight into an erased param + `is T`-check it. Wanted: a regression snippet per boundary (erased call-arg, erased field-store) mirroring the get-return case to lock this down.
    - **Decision to make:** generalise `Box` emission to those sites, OR — better — apply the deeper fix that makes `Box` *unnecessary*: **unify the array-element primitive `BcType` index with the standalone one at intern time** (a primitive currently has TWO indices; dedup them) so no boxing site can ever choose `vader_box_i64(99u,…)` over `…(29u,…)`. The deep fix would let us DELETE the `Box` op rather than spread it across every erasure boundary.
  - **Follow-up: remove the dual for-in fast-path** (`vader/lower/lower_for_in.vader` — `struct_has_field(…"ekeys")` switching compact vs the legacy chaining `lower_map_set_*` branch). It's a transitional reseed bridge; once the seed is fully compact-dict, delete the chaining branch + `struct_has_field`.

- [ ] **🐛 Empty-array `= []` struct-field defaults ALIAS one shared array** (found 2026-06-20, dirty bug — fix soon). When a struct has **multiple** fields defaulting to `= []` (e.g. `ekeys: K[] = []`, `evals: V[] = []`, `index: i32[] = []`), constructing it with the all-defaults literal `Foo {}` makes those fields **share a single empty-array backing** — they alias, so a `push`/index-write to one mutates all of them → silent corruption. **Manifests CROSS-MODULE** : in-module construction gives fresh per-field arrays (works), but a different module's `Foo {}` aliases (caught when the compact-dict `MutableMap` — 3 array defaults — corrupted on cross-module use while its in-module @tests passed). A struct with ONE array default (the old chaining map's `buckets`) never exposed it. **Repro** : `tmp_mappoc/scale` — `MutableMap<i32,i32> {}` cross-module loses all but the first key ; `{ .ekeys=[], .evals=[], .index=[] }` (explicit separate literals) works. **Root cause** : the default-value init for `= []` shares one empty-array constant across the fields (likely a single hoisted/interned `LoweredArrayLit` reused per field default) instead of materialising a fresh array per field per construction. **Workaround in place** : `std/collections::ensure_index` reassigns `self.ekeys/evals/index` to fresh literals on first use — remove once fixed. **Real fix** : in lower / c-emit, emit a fresh array allocation per `= []` field default at each struct construction (no shared constant). Audit other `T[]`/`MutableMap`/`MutableSet`-defaulted structs for the same latent aliasing.

- [ ] **Escape analysis for compiler optimization** (added 2026-06-19, note for later). Study an escape-analysis pass to reduce GC pressure : allocations that provably don't escape their creating function (no store into a heap object, no return, no capture by an escaping closure) can be stack-allocated (or pooled / freed eagerly) instead of going through the Cheney arena. Motivated by the self-compile memory profile — ~840 MB peak RSS, ~498 MB live-set, plus ~340 MB of CFG-churn ; cutting short-lived heap traffic would lower both the retained set and the per-pass churn peak that gates the OOM ceiling. Natural seam : a midir analysis over the CFG (`vader/midir/`) feeding the lowerer / c-emit allocation sites. Scope to study first : which allocation classes dominate (CFG nodes, Type IR, `MutableMap` entries), how precise the analysis must be to pay off, and the interaction with the moving collector (a stack slot must not be a GC root the collector tries to relocate). Architecture-level → explore-before-commit + review.
