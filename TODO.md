...
- [ ] **`i32[]` in let-stmt value position** — minor follow-up. `t :: i32[]` currently traps with `unreachableTypeExprInValuePosition` because the parser emits an `ArrayTypeExpr` AST node and the value-side typechecker doesn't expect it. Same shape works fine in type position. Fix : route value-side `ArrayTypeExpr` through `lowerExprAsType` when the slot is `type`-typed (or just teach the typechecker's expr.ts dispatcher to fold it into TY.type with the resolved ArrayType serving downstream type-position consumers).
