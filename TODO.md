...
- [ ] **Enforce struct-level bounds at instantiation site** — `MutableSet :: struct[T: Hash + Eq] { ... }` parses today but the typechecker accepts `MutableSet[NoHash]` without diagnosing the missing impl, deferring the failure to a downstream crash. Fix : (a) carry the bounds onto the struct's symbol in the resolver (already done for fn type-params), (b) at every struct-type-expr / struct-literal site validate that the type-args satisfy the declared bounds (new diagnostic, e.g. T3034), (c) optional ergonomics — let fns operating on the struct inherit the bound implicitly so methods don't repeat `[K: Hash + Eq]`. Saves the boilerplate that today repeats the bound on every method of `MutableMap`/`MutableSet`.
