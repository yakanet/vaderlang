...
- [ ] **Authoritative resolution (complete symbol-id tracking)** — CADRAGE + POC recording DÉ-RISQUÉ (2026-07-01), plan [`.claude/plans/symbol-id-tracking.md`](.claude/plans/symbol-id-tracking.md). Driver = **fiabilité LSP + simplification** (retrait des 2 fallbacks `symbols_by_name`/`core_symbols` + re-dérivation lower/comptime + accesseurs fragmentés) — **PAS W0006** (indépendant, name-based via `dead_code.vader`). `idents` incomplet pour 3 raisons (clones post-walk, prélude auto-chargé, clé span-hash) ; les fallbacks épongent. POC recording (mémoïsation lazy) : point fixe **tient**, fallback **1458→228**, gap 100% `Span`/`Type` (imports fixes, **0 type-param** → cas fragile structurellement absent). Approche **A** (remplir `idents` via les synthétiseurs). Phase 0 = POC collision (0a unicité globale + 0b oracle différentiel `precise==fallback`). Phase 5 = big-cleanup code mort. `substitute`/`materialize` sont pure-AST → threader le contexte résolu.
