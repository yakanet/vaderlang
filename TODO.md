...
- [ ] **Project-root resolution for `import "vader/parser/ast"`** — `vader/resolver/module.vader::resolve_import_path` only tries stdlib root or `from_file`'s dir. TS has a third tier (`projectRoot`) for absolute project-rooted imports (`import "vader/parser/ast"` from `tests/snippets/*/_main.vader`). Without it, `wire_module` records `missing` for these imports, `imports_to` stays empty, and cross-module type-aliases like `Decl` resolve to `?` in `lower_ident`. Fix : thread `project_root` through `ResolveImportOptions`, fall back to `join_path(project_root, raw)` when raw isn't stdlib/relative. Blocks `namespace_alias_dedupe`. (As a defence-in-depth, `lookup_external_type` now also falls back to the owner's `alias_types` so once wire resolves correctly, the type-alias case is covered.)
- [ ] **Empty-array backwidth from first use** — `arr := []` then `arr.push(1)` should retroactively type `arr` as `i32[]`. TS defers the let-binding's type until first concrete usage. Self-host pins it to `?[]` immediately. Blocks `array_push`. Minimal fix : in `call.vader::infer_call`, after T bindings are settled, if any arg is an `IdentExpr` resolving to a `Local` Symbol with type `?[]` AND T binds to a concrete type, patch `local_types[binding_span] = T[]` and re-walk subsequent expr_types entries that referenced arr. The re-walk is the moderate part.
