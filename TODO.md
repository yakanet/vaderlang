...
- [x] **`if x is T as binding { ... }` now supported** (2026-05-15). Mirrors `match` arms' `is T as a -> …` : the binding lives in the if's then-block, the else/post-if scope doesn't see it. Parser captures `as <ident>` on a `BinaryExpr` with `op = "is"` (new optional `bindAs` field). Resolver introduces a fresh binding scoped to the then-block (new `ifIsBindings` map keyed by the BinaryExpr, mirroring `patternBindings`). Typechecker pushes a per-symbol narrowing for the binding using the same `is`-check type the surrounding flow narrowing uses. Lowerer prepends a `LoweredLet { name = bindAs, value = (T) cond.left }` at the head of the then-block (mirror of `walkPatternBindings`' IsPattern path). Snippet : `tests/snippets/if_is_as_bind/` (direct bind, scope-doesn't-leak case via parity check). The previous "long if-is chains break narrowing past ~80" entry was a *misdiagnosis* — the original cascade came entirely from an invalid `as` binding in an if-condition, whose parser recovery confused downstream `is` narrowing. Touched : `src/parser/passes/expr.ts`, `src/parser/ast.ts`, `src/resolver/{resolve.ts,resolved-ast.ts,symbol.ts,index.ts}`, `src/typecheck/passes/expr.ts`, `src/lower/passes/expr.ts`, plus self-host parity in `vader/parser/{ast.vader,parser.vader,dump.vader}` and `vader/fmt/printer.vader`.
...
- [x] **Brainfuck compiler in Vader, targeting the Vader VM** (2026-05-14, rewritten via builder 2026-05-15). Single-file example `examples/brainfuck.vader` + companion `examples/hello.bf`. Tokenises BF, matches brackets, constructs an `op.Module` programmatically through `vader/vm/builder` (no `.virt` text intermediate), and executes through `vader/vm/exec.exec` — no subprocess, no intermediate file. Helper fn `print_byte(b: i32) -> void` handles `.` output via `int_to_char` + builder intrinsics + `std_io.print`. The builder + writer APIs (`new_module_builder`, `new_function_builder`, `write_virt`, `parse_virt`) are publicly consumable from user code, validating one of the original design goals. Runs on **both** the interpreted path (`vader run`) and the C-emitted native target (`--target=native`) — the early concern that the GC's static type-index table couldn't see runtime-materialised types was a false alarm, since the synthetic `type_id` fields are runtime tags on `StructVal` / `ArrayVal`, not GC type lookups. BF input `,` not wired (no `read_byte` host) — stubbed to "store 0".
