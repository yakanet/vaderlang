...
- [ ] **Minify the C that becomes `bootstrap.c` — a ~12 % lever now, no longer 114 KB** (idea 2026-07-28, measured the same day; **restated the same day** once the seed stopped being gzipped). Stripping leading indentation and blank lines takes the seed from 11.5 MB to 9.1 MB (−20.6 %). Verified end to end: the minified seed compiles with `cc -O0` and the resulting stage0 compiles a snippet correctly.

  **What the storage change did to this idea.** The original headline — 1064 KB → 950 KB gzipped, "114 KB per reseed" — measured the *compressed artefact*, which no longer exists: Phase 1 of `.claude/plans/2026-07-28-seed-size.md` stores the seed raw. The number that replaces it, measured on four real consecutive seeds packed both ways: **1.6 MB raw → 1.4 MB minified**, so ~12 % of a cost that Phase 1 had already cut by 63 %. The two compose (delta compression is byte-based, so it does not care about the reformatting), but this is now a modest follow-on rather than a lever of its own.
