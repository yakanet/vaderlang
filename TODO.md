...
- [ ] **Type-alias unions don't expand in `Alias[]` fn signatures across modules.** `export Value :: I32Val | StringVal` in `vader/vm/op.vader` ; `vader/vm/exec.vader` imports `Value` and declares `fn push_value(stack: Value[], …)`. The typer sees the param as `?[]` (unresolved element) and rejects callers passing a `(I32Val|StringVal)[]`. Work-around : inline the union in fn signatures (`stack: (I32Val | StringVal)[]`). Same-module / single-file uses of `Value[]` are fine ; the bug is the alias-resolution step that skips union expansion when the alias crosses an `import` boundary into a generic-shaped position (array type, probably also `Map<K, V>` etc.). Repro is the entire `vader/vm/` tree post-sprint-3.
