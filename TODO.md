...
- [ ] **UFCS resolves *through* a namespace alias — `x.f()` compiles where the bare `f(x)` is R2006** (found 2026-07-28). With an alias-only import (`AST :: import "vader/parser"`, which puts nothing in unqualified scope), `factory.mint()` type-checks and runs while `mint(factory)` is correctly rejected — so the alias leaks its exports into unqualified *method* position. Confirmed on a struct receiver with a `self`-first fn too (`L.mint(c)` and `c.mint()` both run): a `self` param is not special, SPEC §UFCS — *"there are no methods in Vader, only free functions"*. **This contradicts SPEC** §"UFCS (Uniform Function Call Syntax)", which promises *"UFCS resolution obeys the same import rule as a bare call … exactly as `foo(x)` would"* — the "exactly as" clause is false for alias-only imports. It is also misleading to the reader, which is the point of the rule: an alias is taken to make provenance visible or to settle an R2031 clash, and the method form erases exactly that. **Cause (located):** `populate_imported_fn_overloads` (`vader/typecheck/orchestrate.vader:1529`) walks the module's `import_paths` and pushes every imported module's exported fns into `typed.imported_fn_overloads`; its own comment names the gap — *"`import_paths` (all forms: destructure / namespace / wildcard)"* — the import FORM is not carried, so an alias-only module contributes exactly like a wildcard one. `try_free_fn_ufcs_with_first` (`vader/typecheck/field.vader:999`) then scans that table and finds the name, while the unqualified-name table never got it. Hence the divergence. **Fix direction:** carry the import form (or pass a namespace-only path set) so alias-only modules stop feeding `imported_fn_overloads`. Must keep working: named imports `import "p" { foo }` (handled by the FIRST block of that fn — legitimate), plain wildcards, the always-in-scope `std/core` prelude, and a module imported BOTH as a namespace and as a wildcard. Emit a diagnostic naming the alias ("did you mean `AST.mint(factory)`?") rather than a bare unresolved-method miss, and add a `tests/diag_corpus/` fixture for the new code. **Blast radius: one site** — `vader/resolver/materialize.vader:156`, `i32(factory.mint())` → `i32(AST.mint(factory))`; measured across `vader/`, 148 qualified alias calls against 1 UFCS reaching through an alias. Beware naive greps when re-measuring: comments, lowercase aliases mistaken for receivers (`p :: import "vader/parser"` in `fmt/printer.vader`), and prelude homonyms (`.push()` on a `JsonValue[]!` is not `resolver::push(self: Scope!)`) each inflated an earlier count. The style rule is already written (`.claude/CLAUDE.md` §6 "UFCS — the call form says where the function comes from") but stays convention-only until this lands.
...
- [ ] **Minify the C that becomes `bootstrap.c.gz` — measured at 114 KB per reseed** (idea 2026-07-28, measured the same day). The seed is generated C, versioned gzipped, and re-committed on every reseed — 399 versions in the history so far. Stripping leading indentation and blank lines from it takes the raw C from 11.6 MB to 9.2 MB (−20.5 %) and, more to the point, **the gzipped artefact from 1064 KB to 950 KB (−10.7 %)**. gzip absorbs some of the indentation's redundancy but nowhere near all of it. Verified end to end: the minified seed compiles with `cc -O0` and the resulting stage0 compiles a snippet correctly.

  **Do it as a filter at reseed, not as a `--pretty=false` CLI flag.** The flag was the original shape of the idea, but it means threading a mode through `c_emit`: every `append("\n")` and every indentation site becomes conditional, across an emitter spread over several files, for an output nobody reads except at reseed. A filter in `bootstrap/regenerate.sh` — sed-equivalent, deterministic, three lines — captures the entire gain, leaves `vader build --target=c` readable for the debugging it exists for, and cannot regress the compiler. Going further than indentation + blank lines (collapsing statements onto one line, spaces around operators) buys the rest of the 20 % but would need real C tokenisation to stay safe inside string literals, which is a different project.

  **It is not one filter, it is three sites — and missing one breaks the bootstrap's own guard.** The minified form has to appear on BOTH sides of every comparison:

  1. `bootstrap/regenerate.sh:32` — filter between the emit and `gzip -9`.
  2. `bootstrap/verify.sh:27` — `cmp -s build/bootstrap.new.c <(gunzip -c bootstrap/bootstrap.c.gz)` compares freshly-emitted, UNminified C against the committed seed. Minify only the seed and this reports **STALE SEED on every run**, forever. `verify.sh` runs in CI (`.github/workflows/build.yml`), so that is a permanently red job, and the likely reaction — muting the check — costs more than the 114 KB is worth.
  3. `regenerate.sh:43`, the suggested review diff, compares the decompressed seed against `build/bootstrap.c` and would become pure noise.

  So the filter belongs in one shared place (`bootstrap/minify-c.sh`) called from all three, not inlined in one script. Still small, but the invariant it touches is the one protecting the seed.

  Also worth knowing: minifying makes the seed diff unreadable line-by-line. Arguably it already is at 215k lines of generated C — what gets checked in practice is the numstat and the fixed point — but say so in the script rather than leaving a suggestion that no longer works.
