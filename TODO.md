...
- [ ] **UFCS on union receivers.** Today `value.read_i32(...)` on a `Value :: I32Val | StringVal` receiver fails to resolve : Vader's method-lookup walks each variant looking for a field/method, never falls back to a free fn whose first param matches the union. Surfaced while writing the self-host VM (`vader/vm/exec.vader`) where every stack-slot is `Value` — the work-around is to call `read_i32(value, ...)` in function-call form. Should align with the existing UFCS on concrete types (`s.byte_len()` where `byte_len` is a free fn taking `string`). Touches the typechecker's method-resolution pass : after the per-variant field/method probe fails, try a free-fn-with-matching-receiver-type lookup, matching against the *union* type (not each variant separately).
- [ ] **Tuple destructure after `[T, U] | null` narrow fails.** Surfaced at sprint 4 (b1) writing `vader/vm/parser.vader`. Pattern : `fn helper(...) -> [i32, i32] | null { ... return [a, b] }` ; caller `pair :: helper() ; if pair is null { return null } ; [type_id, field_index] :: pair` — Vader rejects the destructure with `P1003: expected an expression: got '::'`, even though `pair` should be narrowed to the non-null `[i32, i32]` after the early-return. The narrowing for `is null` works fine for scalar use (`pair.method()` would compile if methods existed) but not for tuple destructure. Work-around : declare a named 2-field struct (`TypeFieldPair :: struct { type_id, field_index }`) and access fields by name. Once fixed, `parser.vader`'s `TypeFieldPair` + `parse_type_field_pair` can collapse back to a tuple-returning helper. Touches : narrowing flow → destructure-pattern path in the typechecker.
- [ ] **Type-alias unions don't expand in cross-module `Alias[]` array slots.** Refined repro after sprint 4 (b1) : the bug is **specifically** about *array element* positions. `export Value :: I32Val | StringVal | StructVal` in `vader/vm/op.vader` ; `vader/vm/exec.vader` imports `Value` and uses it. Scalar positions resolve correctly cross-module — `fn read_i32(v: Value, …)`, `return_value: Value = …`, `HostOk.value: Value` all work. **Array positions fail** : `fn push_value(stack: Value[], …)` or `slots: Value[] = []` triggers `T3001: expected ?[], got I32Val | StringVal | StructVal[]` — the alias expands but with wrong precedence (only the last variant gets `[]` instead of the whole union). Explicit parens (`(Value)[]`) don't fix it. Same-module / single-file uses of `Value[]` are fine. Work-around : inline the union in array positions (`stack: (I32Val | StringVal | StructVal)[]`) ; scalar positions can stay as the alias. Repro is `vader/vm/exec.vader` Frame.slots/stack, push_value, build_callee_frame, dispatch_import.
