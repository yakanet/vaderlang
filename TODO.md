...
- [ ] **CI review** (added 2026-08-27, user request). Concrete gaps, measured on runs #208-#214:
  ...
  - **Job durations to keep an eye on** (main #208 → branch #214): Test 6m55 → 8m18, Test (Windows) 8m16 → 11m52. The delta is the `hook_fixtures` corpus — six distinct build drivers, one whole-compiler compile each. Decide whether that corpus runs on every push or on demand like `fixed-point`.
