...
- [~] **Namespace-alias struct-literal field resolution** (surfaced 2026-05-24, struct portion fixed same day). Root cause : multi-file modules (e.g. `vader/vm` with 6 files) only had `declare_module` called on `files[0]`, so structs declared in non-first files (`vader/vm/op.vader::PrimitiveType` — `builder.vader` sorts first) never landed in `struct_decls` / `struct_field_types`. Fixed via new `declare_extra_file_type_decls` pass in `vader/typecheck/decl.vader` that walks sibling files and populates ONLY the Symbol.id-keyed side-tables (`struct_decls`, `struct_field_types`, `trait_decls`, `trait_decl_owners`). The repro `alias.PrimitiveType { .kind = "i32" }` now typechecks. **Still outstanding** : FnDecls and ConstDecls from sibling files. These write to `decl_types` (span-keyed by `(line, col)`) which collides across files — a fn at `(575, 8)` in builder.vader clobbers the entry for a fn at `(575, 8)` in exec.vader. Symptom : `lookup_external_type` (`orchestrate.vader:1022`) returns the wrong fn type for cross-module fn imports defined in non-first files. `builder_roundtrip` still hits this via `exec(...)` (defined in `vader/vm/exec.vader`) — currently typed as `fn(FunctionBuilder) -> void` because `drop` in `builder.vader` shares (575, 8). Proper fix is re-keying `decl_types` by `Symbol.id` ; tracked separately because it touches ~15 read/write sites and the TS pipeline already keys by AST-node identity (no collision possible there).
