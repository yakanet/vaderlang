...
- [ ] **Vendor a validated cross-platform layer for the low-level io, instead of hand-rolling `#if _WIN32`** (added 2026-08-27). `runtime/c/vader_runtime.c` carries **28 `_WIN32` conditionals** in 4445 lines, covering `create_dir`, `remove_file`, `read_dir`, `exists`, `is_dir`, `temp_dir`, `current_executable_location`, `current_working_directory` and the `spawn_*` family. **Two Windows-only correctness bugs came out of that surface in one week**: `create_dir` accepting a *file* as its target, and its prefix walk trying to create the **drive root** `C:` — which fails when the process's working directory is on another drive, exactly the GitHub-runner shape (`D:\a\...` cwd, `C:` temp path). Both were invisible on POSIX. **Surveyed 2026-08-27 — and the answer inverts the recommendation.** `nothings/stb` itself ships **no** filesystem or OS/IO library: all 21 are domain-specific (images, audio, fonts, packing, `stb_ds`, `stb_sprintf`, `stb_c_lexer`…). Its curated companion list (`nothings/single_file_libs`, `_file_` / `_sys_` sections) has, against our surface: **TinyDir** (C, BSD — "cross-platform directory reading, Win/POSIX/MinGW") for `read_dir`, **Whereami** (C, WTFPL2 — "get path/filename of executable or module") for `current_executable_location`, and **fs** (C, PD/MIT-0 — "file system and archive abstraction, Win/Mac/Unix") for reads/writes/`exists`/`is_dir`. But **nothing covers `mkdir -p`, `temp_dir`, or process spawning** — and those are exactly where both bugs were: the drive-root walk in `create_dir`, fed by `temp_dir`. **So vendoring would not have prevented either defect.** `fs` is the only multi-concern candidate and it is an abstraction layer with archive support (zip/pak/RAM): adopting it means adopting its model, not importing a function. Conclusion: the library question is worth at most TinyDir + Whereami (two narrow, audited replacements for ~60 of the 200 lines), and it is NOT the answer to the correctness problem. **The testing gap is the sharper half**: nothing exercises the `_WIN32` branches except CI, where `test-windows` is `continue-on-error: true` and can rot silently. Cross-compiling with mingw-w64 and running under wine DOES work — it verified both fixes above — and would make a cheap always-on job.
