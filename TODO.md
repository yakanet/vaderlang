...
- [ ] **Open-addressed hash table for `MutableMap`** (companion to next()-inline). The chaining design (`buckets: (Entry | null)[]` with linked-list `next` pointers) costs an extra ref deref per probe + an extra alloc per insertion ; open addressing (linear probing with Robin Hood + power-of-two sizing) keeps everything in one flat array, halves the per-lookup ref hops, and removes the `Entry.next` allocation entirely. Trade-offs : (a) Robin Hood is ~80 LoC of Vader vs the current ~50, (b) tombstones on delete (current chaining naturally compacts), (c) iterator stability across rehash (callers must complete iteration before any insert). Most likely lands together with next()-inline since both target map_iter's hot path. Expected combined gain : Vader within ~1.2 × of Go on map_iter. **Attempt 2026-05-16** : Robin Hood Vader impl + Vader-side iter rewrite worked on TS VM (snippets green, perf gain) but native silently dropped data — `self.entries[idx] = Entry { ... }` in the probe loop didn't persist through the C-emit path. Reverted. Two distinct compiler bugs surfaced and need their own audits before re-attempt : (1) cross-module struct field defaults : `cap_mask: usize = usize(0)` works in the defining module but `MutableMap(i32, i32) {}` from a caller hits `LoweredUnreachable` because the user's `resolved.idents` doesn't have the AST nodes of the default expression ; (2) `array.set` of an Entry struct ref in tight loops doesn't observably mutate the array on native target (suspicion : `slot :: self.entries[idx]` rebinding captures a stale copy ?).
