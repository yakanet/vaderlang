...
- [ ] **`std/iter` combinators** (`map / filter / take / skip / fold / sum / count / collect`). Per SPEC §15 these are MVP scope. Today `count` and `collect` exist with bodies in `stdlib/std/iter.vader`, but **no call site can bind `Iterator($T)` to a concrete iterator** (e.g. `MutableListIter(i32)`). This needs **trait-based unification** in `unifyTypeParam` (`src/typecheck/passes/call.ts`) so that an arg of type `Struct(S, [args])` can satisfy a param of type `Trait(T, [vars])` when `S implements T(args')` — bindings flow through the impl's type-args. Plus the related transitive monomorphization (next bullet).
