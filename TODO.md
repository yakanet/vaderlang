...
- [x] **Generic `Range`** — `Range :: struct[T] { start: T, end: T, inclusive: bool, cursor: T }`. `Range[i32]` / `Range[char]` impls of `Contains` and `Iterator` ship in std/core ; `inferRange` is bound-type-driven (i32 or char today). char arithmetic for the iterator advance uses an explicit `char(u32(self.cursor) + u32(1))` round-trip since `char` has no native `+ i32`. Stdlib char-class predicates (`is_digit`, `is_alpha`, …) migrated to `c in '0'..='9'`. Snippet : `tests/snippets/char_range_contains/`. **Other integer widths** (`usize`/`u64`/etc.) deferred until the call needs them — the bound-type detection already accepts them syntactically, the impl-table lookup just doesn't have a matching impl yet.
...
- [ ] **Auto-inherit struct bounds onto methods** — today `fn put :: fn[K: Hash & Eq, V](self: MutableMap(K, V), ...)` repeats the bound that `MutableMap[K: Hash & Eq, V]` already declares. Sketch : at typecheck time, after `declareModule`, walk every fn whose first parameter resolves to a generic struct instance whose type-args reference the fn's own type-params positionally ; copy the struct's typeParam bounds onto the matching fn typeParams. Implemented in a small extra pass between `inferExprBodiedReturns` and `checkProgram` so the body's `key.hash()` already sees the inherited bound. Implementation is ~50 lines in a new `passes/inherit-bounds.ts` plus widening `Globals.typeParamBounds` to `Map` (the underlying allocation is already mutable, only the type signature is `Readonly`). Estimate : ½ day. Removes the `[K: Hash & Eq, V]` repetition on the 3 method signatures that currently carry it in `std/collections.vader`, plus a similar saving on any future generic-struct-with-bounds API.
...
- [ ] **8d — default-method injection at `implements` site** — not supported today : a trait method body (`fn not_equals = !self.equals`) is parsed but not injected into impls. Layer 8b's T3036 fires for any trait method missing in the impl block, regardless of whether the trait provided a default. Implementation needs `materializeDefaultMembers` similar to `materializeIntrinsicMembers` / `materializeSamMembers`, plus a decision on body sharing (clone vs. share node) — sharing is simpler but couples the `self` substitution across the trait + every impl ; cloning preserves independence at the cost of an Expr/Block deep-copy helper.
...
- [ ] **8f — same-signature method satisfies multiple traits** — partial today. `Cat implements Greet { fn greet }` and `Cat implements Annot { fn greet }` both work (each impl block is its own scope, distinct mangled fns). Different-signature `greet` impls coexist without diagnostic. Ambiguous `c.greet()` call silently picks the first-declared impl. Strict design-doc conformance would diagnose the second impl with a different signature, and error on ambiguous call sites — needs cross-impl method-name tracking in the impl registry.
...
- [ ] **`match enum` cascade → `switch` C** (Prop 1). The lowerer expands `match e { .V0 -> ..; .V1 -> ..; .. }` over a primitive enum into a nested `if (e == K0) { .. } else { if (e == K1) { .. } else { .. } }` cascade with a per-level result-propagation chain (`local.get inner_slot; local.set outer_slot` after every `end`). `cc -O3` recognises dense cascades and rebuilds a jump table, so this is mostly a code-size / compile-time / readability win — but it's substantial : `vader_diagnostics_codes_vader_info` (10 variants) emits 22-deep nesting and ~140 lines of cascade where a 30-line `switch` would do. The clean approach is bytecode-level : add an `op.switch.table { scrutSlot, cases: (K_i, body_op_range_i)[], default_op_range }` op, detect the pattern in the lowerer (or as a post-bytecode peephole) when (a) the match scrutinee is a primitive enum local, (b) every arm pattern is a single bare variant, (c) bodies are flat (no nested control), and replace the cascade + propagation chain with the new op + a single result slot every arm assigns directly. Then add VM + C-emit handling. C-emit converts to native `switch (lS) { case K_i: ...; break; default: ...; break; }`. The propagation-chain rewrite is the load-bearing part — bypass attempts at the C-emit level alone are fragile because the per-level result slots are interleaved with the if/else structure. Estimate ~1 day for the bytecode approach + tests + snapshot refresh. Pre-requisites already landed : Rule 6 (const-prop, Layer #17) makes the pattern syntactically clean ; Prop #16 (no-alloc frame skip) means converted fns can also drop their gc_frame.
- [ ] **Static lookup table for `match enum -> StructLit constant`** (Prop 2). Pattern : an expression-bodied fn whose entire body is `match c { .V0 -> Foo { .a = "lit", .b = 1 }; .V1 -> Foo { .. }; .. }` over a primitive enum, with EVERY arm yielding a `StructLit` whose fields are all compile-time constants. Today : 23 `vader_gc_alloc` per call (`parser_info`), one per variant. Target : single static table + indexed lookup, zero allocation. Implementation : (1) detect the pattern in the lower (`expr.ts MatchExpr` of an expression-bodied fn) ; (2) collect the table into a new `BytecodeModule.staticTables: readonly StaticTable[]` slot ; (3) replace the fn body with a single new op `op.static_table.lookup tableId` that pops the index and pushes a ref ; (4) C-emit translates to `static const T table_N[] = { .. }; return vader_box_obj(tag, (void*)&table_N[idx])` ; (5) VM mirrors with a JS-side static table. Pre-requisite : the GC must skip pointers outside the from-space arena (5-line addition to `vader_gc_forward` — return `obj` unchanged when `(char*)obj < g_from_space.base || >= g_from_space.end`). The static structs cannot transitively reference dynamic objects (they would never be scanned) ; the detection MUST gate on "all fields are primitive literals or static-string-ref". Bonus : the resulting fn becomes no-alloc → Prop #16's frame-skip kicks in too. Massive impact for the diagnostic-info family (`info`, `parser_info`, `resolver_info`, `typecheck_info`, ...). Estimate ~1 day.
- [ ] **Eliminate single-use primitive temp locals introduced by struct-literal lowering** (Prop 3). The lowerer creates a fresh local per struct-literal field for evaluation-order safety. For ref-typed fields this is necessary (GC pin), but for primitives (incl. `string`) the local is dead after one read and the pre-storage `lN = vader_str_X;` followed by a single `field = lN;` is pure noise. Two options : (a) tighten the lower for `StructLit` to skip the temp when the field expr is side-effect-free (constants, locals, arithmetic) ; (b) add a peephole rule "T.const K ; local.set N ; ... [no other ref to N] ... ; local.get N → ... ; T.const K". (a) is local and safer ; (b) is global and trickier (need stack-discipline-aware data-flow). Combined with Prop 2, eliminates ~46 lines per `info`-style fn. Estimate ~½ day for (a).
