...
- [ ] **`T[]` should implement `Index` / `IndexSet`** (2026-07-20). Arrays index via a built-in fast path (`arr[i]` / `arr[i] = v` lower directly), but `T[]` does not carry the `Index<usize, T>` / `IndexSet<usize, T>` traits, so an array can't satisfy a `[C: Index]` generic bound. Declaring `@intrinsic T[] implements Index/IndexSet` (attempted, reverted) is inert for real code but introduces a footgun: it makes the bound typecheck-accept the array, then panic at emit (GATE B1 — the bodyless `at`/`set_at` members aren't materialised into a callable body). TWO blockers to clear first: **(1)** the index OPERATOR `c[0]` does NOT dispatch through a generic trait bound — `infer_index` (`vader/typecheck/expr_index.vader`) rejects a `TypeParamType` target with T3008 "not indexable" even for a custom struct with an `Index` impl (the METHOD form `c.at(0)` already works via `try_typeparam_bound_method`; the operator doesn't) — fix by routing a TypeParam-`Index`-bound target through the bounded-method machinery (typecheck + `lower_index` vcall); **(2)** the array's bodyless intrinsic `at`/`set_at` need a materialised body (a wrapper emitting the built-in indexed op) so an erased/vcall dispatch resolves. Companion to the landed `@intrinsic ArrayOps` array-methods conversion (`stdlib/std/core/array.vader`); doing this extends that model to indexing. Blocker (1) is independently useful (generic index-operator dispatch for any user `Index` type).
