...
- [ ] **Inline the trivial `infer_*_lit` literal-inference fns** (noted 2026-06-22, user-reported). After the typed-suffix removal, `infer_int_lit`/`infer_float_lit`/`infer_bool_lit`/`infer_null_lit`/`infer_char_lit`/`infer_string_lit` (`vader/typecheck/expr_literals.vader`) all ignore their `AST.*LitExpr` argument and just return a constant (`TY_FREE_INT`, `TY_FREE_CHAR`, …). The dispatch in `expr.vader::infer_expr` could return the constant directly (`is AST.IntLitExpr -> TY_FREE_INT`) and drop the wrappers — fewer indirections. Low-priority cleanup; check each (the StringLit one walks interpolation parts in `infer_expr`, not in `infer_string_lit`, so verify nothing else relies on the fn boundary).

- [ ] **Array rest-destructuring `[head, ...rest] = items` may not work** (noted 2026-06-22, user-reported). The spread/rest element in a destructuring pattern (e.g. `[it, ...rest] = items` to bind `it` = first element and `rest` = the remaining slice) appears unsupported — the parser only has element-wise `[p1, p2, ...]` tuple-destructure patterns (`ast.vader:688`), no rest binding. Confirm whether it was ever implemented (user thought so); if not, decide whether to add it (parser pattern + typecheck slice type + lower to head + tail-slice). Investigate before acting.
