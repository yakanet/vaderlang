...
- [~] **LSP semantic classification via the AST** (partial — 2026-05-16). `vader/lsp/ast_tokens.vader` already walks the parsed AST and tags `IdentExpr` by position (callee of CallExpr → `Function`, type-position → `Type`, param decl → `Parameter`, etc.). The 2026-05-16 patch added a primitive-name set so `i32.MAX` paints `i32` as `Type` and `i32(x)` casts paint `i32` as `Type` (not `Function`). **Remaining gap** : in-body scoping — `x` (param) and `val` (local) at use sites are both tagged `Variable` because there's no resolver-driven scope tracking. **Next step (architectural)** : port body-walking resolution into `vader/resolver/resolve.vader` so the compiler AND the LSP share the same `IdentExpr → Symbol` map. The current `vader/resolver/` only collects top-level decls ; extending it to walk fn bodies + blocks + lambdas with a scope stack (~200-300 LoC) yields a `resolved.idents` table that both the compiler-to-come and `ast_tokens.vader` consume. Avoids duplicating the walker. Acceptance : in-body params paint `Parameter`, locals `Variable`, imports per their decl kind ; reuses the same resolver invoked by compilation.
