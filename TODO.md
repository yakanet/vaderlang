...
- [~] **Resolver port follow-ups** (large pass 2026-05-25, parity-diagnostics now 1151 pass / 26 skip / 0 fail). Closed items :
  ...
  Remaining open (tracked by `tests/parity-diagnostics.test.ts::KNOWN_DIVERGENCES`, 8 entries — full suite 4998 pass / 14 skip / 0 fail) :
  - `iter_defaults` / `selfhost_lexer_basic` typecheck-coverage (6 entries) : Vader's typechecker doesn't populate `expr_types` for as many nodes as TS (visible as `?` in fn signatures of the dump). Diffuse fix per arm of `walk_expr` ; needs an audit pass to enumerate the missing writes. Root cause traced to `expr_if.vader::collapse_if_unresolved` propagating UnresolvedType when any branch is `?`.
  - `tuple_comptime` (2 entries) : `@comptime DIM : [i32, i32] : [10, 20]` needs SeqLitExpr eval *with* expected-type context to distinguish tuple from array. The current tree-walker has no expected-type plumbing.
