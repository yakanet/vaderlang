...
- [x] **Module system redesign** (landed 2026-05-22). Explicit `module "..."` keyword on every `.vader` file ; module identity decoupled from filesystem layout. Strict resolver (TS side) replaces the legacy `promoteToFolderModule` heuristic with an up-front `discoverModules` scan + name-indexed import lookup. Stdlib reorganised into per-module folders ; vader/types + vader/resolver/symbol extracted to break the typecheck↔resolver cycle that surfaced at the strict flip. Design log + 42 decisions in [`docs/MODULE_SYSTEM.md`](./docs/MODULE_SYSTEM.md). **Deferred** : (a) full strict-flip of the Vader self-host's own resolver (codes + `discover.vader` shipped as additive infrastructure, full BFS rewrite postponed — `vader/resolver/loader.vader` is MVP-depth-1 anyway), (b) parser `module "..."` strict-presence diagnostic (resolver R2020 already catches missing decls at scan time), (c) TS-vs-Vader byte-diff in `parity-broad.test.ts` (smoke check stays ; a scratch comparison surfaced ~38 cross-implementation divergences that need a separate chantier).
...
- [ ] **Non-owning slice / span views** (gap surfaced 2026-05-19) — today `arr[r]` (typecheck `expr.ts:374-386`) and `string.slice()` (`stdlib/std/string.vader:30`) both **allocate a fresh copy**. Add a first-class non-owning view type — `Slice[T]` over arrays, `Substring` (or unified `StringView`) over strings — as a `(ptr, len)` pair that aliases the backing buffer. Use cases : parser hot paths, codec / regex inner loops, log scanners, anything that today is forced to copy just to look. Touches : (a) type IR + checker (new type-kind or alias of `Span[T]`), (b) lower + bytecode (no copy, just retag), (c) C-emit (already trivially `(ptr, len)`), (d) GC rooting (slice keeps its parent buffer alive). Open : syntax (`arr[r]` returns a view vs an array — breaking semantic change ; or `arr[r as Slice]` opt-in), and the codepoint-vs-byte question for `Substring` interacts with the string-model rework above.
