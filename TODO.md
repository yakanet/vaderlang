...
- [ ] **Default-method materialize into impls with original line:col** — TS materialises trait default-bodied methods (`Iterator.is_empty`, `Iterator.count`, …) INTO each implementing struct's impl, preserving the std/core line:col in the dump but with `Self → struct_ty` + `T → trait_arg` substituted. Blocks `custom_iter`, `custom_iter_generic`, `iter_defaults`, `trait_dispatch_generic_iter`, `for_in_iter_trait`. Heavy : needs a body-walk pass that types the trait method's AST under a substituting context AND writes to entry's typed program at the trait method's original spans.
- [ ] **T3006 (trait bound not satisfied) at call sites** — `double(v)` where `double :: fn[T: Add](x: T)` and `Vec3` doesn't implement `Add` should emit T3006. Needs the call site to look up the callee FnDecl (to read `type_params[i].bound`) and check each bound against the bound arg. The dispatch reordering in field.vader makes the expr types right ; only the diagnostic is missing. Stub : add a `fn_decls: i32 → FnDecl` side-table populated at declare-time, then check bounds in `call.vader::infer_call` after the unify loop. Blocks `conformance_explicit`.
...
- [x] **Cross-module type alias unions** — collateral win from the cross-module folder-module fix (2026-05-17). `alias_union_in_array` + `namespace_alias_dedupe` both pass now that mod loading completes and aliases reach `external_types` properly.
