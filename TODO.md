...
- [ ] **`match enum` cascade → `switch` C** (Prop 1). The lowerer expands `match e { .V0 -> ..; .V1 -> ..; .. }` over a primitive enum into a nested `if (e == K0) { .. } else { if (e == K1) { .. } else { .. } }` cascade. `cc -O3` recognises dense cascades and rebuilds a jump table, so this is mostly a code-size / compile-time / readability win — but it's substantial : `vader_diagnostics_codes_vader_info` (10 variants) emits 22-deep nesting and ~140 lines of cascade where a 30-line `switch` would do. Two clean approaches : (a) bytecode-level — add an `op.switch.match { kvs: (K_i, body_ip)[], default_ip }` op, generate it from the lower when the match is over a primitive enum with constant patterns, and add VM + C-emit handling ; (b) C-emit-only — pre-walk each fn body to identify cascade chains (a `local.get S; X.const K; X.eq; if void` quad followed by a same-shape chain in the else), mark IPs as switch/case/break/end, and rewrite emission. (a) is cleaner and benefits the VM too ; (b) doesn't touch the bytecode but is single-pass-fragile. Estimate ~½ day for (a), ~1 day for (b).
