...
- [ ] **`Range` is i32-only ; `for i in 0..<n` with `n: u64` traps at runtime instead of erroring at compile time.** Repro: `examples/fibonacci.vader` (uses `n: u64`) → VM trap `vm: expected numeric, got i64` (also reproducible with a 5-line script). Two compounding bugs: (a) `std/core::Range` is hard-coded with `i32` `start`/`end`/`cursor` fields (`stdlib/std/core.vader:53-58`), so a u64 bound silently lands in an i32 slot via `lowerRangeExpr` (`src/lower/passes/for-in.ts:25`); (b) `inferRange` (`src/typecheck/passes/expr.ts:240-249`) calls `checkExpr(.., TY.i32, ..)` but `checkExpr` doesn't enforce `isAssignable(got, expected)` — it only uses `expected` for free-literal defaulting (`src/typecheck/passes/expr.ts:38-41`), so a u64 expression slips through unflagged. **Quick fix (~5 lines):** add an explicit `isAssignable` check inside `inferRange` so non-i32 bounds surface as a `T3xxx` diagnostic instead of a VM trap. **Proper fix:** generalise `Range :: struct($T)` over the integer type with impls for i32 / u64 / usize, mirroring `Yielded(T)` and `ArrayIter(T)` (cf. SPEC §1442 listing `Range` alongside the generic iter types). Bigger lift — touches `std/core`, `inferRange`, `lowerRangeExpr`, and the resolver's `CORE_STRUCTS` registration.
