...
- [ ] **Open-addressed hash table for `MutableMap`** (companion to next()-inline). The chaining design (`buckets: (Entry | null)[]` with linked-list `next` pointers) costs an extra ref deref per probe + an extra alloc per insertion ; open addressing (linear probing with Robin Hood + power-of-two sizing) keeps everything in one flat array, halves the per-lookup ref hops, and removes the `Entry.next` allocation entirely. Trade-offs : (a) Robin Hood is ~80 LoC of Vader vs the current ~50, (b) tombstones on delete (current chaining naturally compacts), (c) iterator stability across rehash (callers must complete iteration before any insert). Most likely lands together with next()-inline since both target map_iter's hot path. Expected combined gain : Vader within ~1.2 × of Go on map_iter.
