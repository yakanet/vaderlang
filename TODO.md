...
- [ ] **Generalise the auto-coerce mechanism (`Display` is the first instance).** The lowerer currently ships one ad-hoc coercion site per trait : `[T] → Iterator(T)` (auto-wrap into `ArrayIterator`) and `T → Display` (static dispatch through `to_string` ; `lower/passes/display-coerce.ts`). Both follow the same shape — typecheck records the source type at the call site, the lowerer rewrites the argument before the host receives it. Worth lifting that into a generic facility :
