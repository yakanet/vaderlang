...
- [x] **`vader run <file>` runs the named file's `main`, not an arbitrary directory entry** (fixed 2026-06-20). The folder-module model means `vader run dir/foo.vader` loads `dir`'s whole module (every sibling `.vader`), and the VM's default entry was the first `is_main` fn (`reduce_main_index`) — blind to which file was named, so `vader run dir/b.vader` silently ran `dir/a.vader`'s `main`. T3053 (signature-keyed) already caught same-signature duplicate `main`s; the gap was two `main`s of *different* signatures (`fn()` vs `fn(argv)`) — both stay `is_main`. **Fix** (`vader/cli/main.vader::entry_index_for_file` / `named_file_main_mangled`, CLI-only — no pipeline/seed change): `run_source` now resolves the entry index by matching the named file's `main` (via the lowered fn's `origin.decl` span file, basename-matched) and execs from it via `exec_entry`, falling back to the first `is_main` when the named file declares none. Single-main modules are byte-identical (verify.sh fixed-point + seed unchanged). Regression: `tests/vader_run_entry.test.ts` (two sibling `main`s, distinct exit codes). Scope: `vader run` only — `vader build` / `dump --stage=c|bytecode` still use the conventional first-`main` (consistency follow-up if wanted).
