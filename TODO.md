...
- [ ] **Call-site function names lose their color under the LSP** (added 2026-07-25). With the LSP driving highlighting, function names in call position (`out.push(…)`, `mk_primitive("usize")`) render in the default foreground (white), while the shared TextMate grammar paints them blue. Same family as the string-escape item above — a semantic token overriding a TextMate scope the theme knows how to color. **The token *kind* looks right on the server side**: `walk_call` (`vader/lsp/handlers/ast_tokens.vader:460`) pushes `.Function` on the callee ident, on `f.field_span` for a method call, and on a `GenericInstExpr` callee ; `TokenType.Function` is legend index 2 → `"function"` (`vader/lsp/protocol/tokens.vader`). The grammar side uses `entity.name.function.call.vader` (`editors/common/vader.tmLanguage.json`, `functions` patterns). So the likely gap is the **client-side mapping**, not the emitted type: the IntelliJ theme has a colour for the `entity.name.function.call` scope but the standard `function` semantic token maps to a `TextAttributesKey` that resolves to the default foreground — the plugin (`editors/intellij/src/main/kotlin/dev/vaderlang/intellij/`) declares no semantic-token → attributes mapping at all. **Next step:** dump the tokens for the screenshot's line to confirm `function` is actually emitted there (rather than `variable` via a fallback path), then either add an explicit semantic-token colour mapping in the plugin, or drop `function` tokens for call sites the way identifiers are already left to TextMate. Check VS Code too — the same emission may already be blue there, which would confirm it's a client mapping issue.
