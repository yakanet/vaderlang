...
- [ ] **Namespace-alias struct-literal field resolution** (surfaced 2026-05-24 after GC default bump). When a snippet uses `alias :: import "vader/vm"` then `alias.PrimitiveType { .kind = "i32" }`, the self-host typechecker emits a spurious T3009 "field `kind` does not exist on PrimitiveType" even though the struct decl has that field. TS resolves cleanly. Blocks `builder_roundtrip` and `peephole_rules` diag-parity (currently in `KNOWN_DIVERGENCES`). Likely a cross-module struct-field lookup that doesn't follow the namespace alias to the target module's declared fields ; mirror the `namespace_member_types` machinery (already used in type-position) into the struct-literal arm.
