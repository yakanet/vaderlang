...
- [ ] **Split the low-level io by concern and platform — don't vendor** (added 2026-08-27, surveyed the same day). `runtime/c/vader_runtime.c` is **4445 lines** mixing GC, strings, arrays, io and process, with **28 `_WIN32` conditionals interleaved** through it. That surface produced **two Windows-only correctness bugs in one week**: `create_dir` accepting a *file* as its target, and its prefix walk trying to create the **drive root** `C:`, which fails when the process's working directory is on another drive — exactly the GitHub-runner shape (`D:\a\...` cwd, `C:` temp). Both invisible on POSIX.
  - **Vendoring is not the answer, checked.** `nothings/stb` ships **no** filesystem or OS/IO library (all 21 are domain-specific). Its curated `single_file_libs` list offers three narrow candidates — **TinyDir** (C, BSD) for `read_dir`, **Whereami** (C, WTFPL2) for the executable path, **fs** (C, PD/MIT-0) for reads/writes/`exists`/`is_dir` — but **nothing for `mkdir -p`, `temp_dir` or process spawning**, which is exactly where both bugs were. So a vendored header would have prevented neither. `fs` is the only multi-concern option and it is an abstraction layer with archive support (zip/pak/RAM): adopting it means adopting its model.
  - **Odin is the precedent worth copying, and it vendors nothing.** Two layers: `core/sys/windows/` is raw Win32 bindings, one file per DLL (`kernel32.odin`, `ntdll.odin`, `advapi32.odin`, …); `core/os/` is the portable API split as **concern × platform** — `dir_windows.odin` / `dir_posix.odin` / `dir_linux.odin` / `dir_posix_darwin.odin`, and likewise `file_*`, `path_*`, `stat_*`, `pipe_*`, `process_*`, `env_*`, `errors_*`, and a dedicated `temp_file_*` per platform. They have a whole file for the concern that fed both our bugs.
  - **Direction**: extract the io/process concerns out of `vader_runtime.c` into per-concern, per-platform C files (`runtime/c/os/dir_windows.c`, `dir_posix.c`, `mkdir_windows.c`, …) so a Windows implementation is a file that can be read and reviewed whole, instead of a branch buried mid-file. Optionally take TinyDir + Whereami for two of the cells (~60 of the ~200 lines). Cheap to sequence: one concern at a time, each a pure move.
  - **The testing gap is the half that actually bites.** Nothing exercises the `_WIN32` branches but CI, where `test-windows` is `continue-on-error: true` and can rot silently. Cross-compiling with mingw-w64 and running under wine works — it verified both fixes above, and `wine vader.exe test vader/hooks` runs the whole module — so an always-on job is available today. It would not have caught the drive-root bug (wine's `GetFileAttributesA("C:")` succeeds), which is itself worth knowing: wine is a good regression net, not a Windows oracle.
