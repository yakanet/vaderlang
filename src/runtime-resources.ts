// Locate the stdlib and C runtime regardless of whether we run as
// `bun src/index.ts` (dev) or as a compiled binary shipped in an archive
// (sidecar). The probe order is fixed, the first hit wins, and the result is
// memoized — no need for a "are we packaged?" flag at call sites.
//
...
function probe(): RuntimeRoots {
  // 1. Sidecar mode: stdlib + runtime/c sit next to the binary.
  ...
  if (
    ...
    return { stdlibRoot: sidecarStdlib, cRuntimeRoot: sidecarCRuntime };
  }
  ...
  if (existsSync(resolve(devStdlib, "std", "io.vader"))) {
    return { stdlibRoot: devStdlib, cRuntimeRoot: devCRuntime };
  }
  ...
  return {
    stdlibRoot: resolve(process.cwd(), "stdlib"),
    cRuntimeRoot: resolve(process.cwd(), "runtime", "c"),
  };
}
