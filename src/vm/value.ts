...
export interface I64Value    { readonly tag: "i64" | "u64" | "usize" | "isize"; readonly n: bigint }
...
export function asBig(v: Value): bigint {
  if (v.tag === "i64" || v.tag === "u64" || v.tag === "usize" || v.tag === "isize") return v.n;
  throw new Error(`vm: expected i64/u64/usize/isize, got ${v.tag}`);
}
...
export function asIndex(v: Value): number {
  if (v.tag === "i64" || v.tag === "u64" || v.tag === "usize" || v.tag === "isize") return Number(v.n);
  return asNum(v);
}
