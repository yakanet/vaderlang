...
export class WasmEmitter {
    ...
    emitTopLevelStatement(statement: Resolved<Statement>) {
        switch (statement.type) {
            case "FunctionDeclaration": {
                console.log('parameters', statement.parameters.map((t) => mapBinaryenType(t.type)))
    ...
    private emitExpression(
        ...
        switch (expression.type) {
            case "VariableExpression": {
                debugger
                const ref = expression.scope.lookupVariable(expression.value[0]);
                const source = ref.source;
                switch (source.kind) {
                    case 'FunctionParameterSource': {
                        return this.module.local.get(source.index, mapBinaryenType(ref.type));
                    }
            ...
            case "NumberExpression": {
                ...
                if (expression.variableType.name === 'f32') {
            ...
            case "BinaryExpression": {
                ...
                if (expression.operator === "*") {
                    return this.module.f32.add(
                        this.emitExpression(expression.lhs),
                        this.emitExpression(expression.rhs)
                    );
                }
                if (expression.operator === "/") {
                    return this.module.f32.div(
                        this.emitExpression(expression.lhs),
                        this.emitExpression(expression.rhs)
                    );
                }
                throw new Error(
                    "BinaryExpression " + expression.operator + " is not implemented."
                );
            }
        }
    ...
    binaryOperations = new Map<
        [string, number, number],
        (a: number, b: number) => number
    >([
        [["+", binaryen.i32, binaryen.i32], this.module.i32.add],
        [["+", binaryen.f32, binaryen.f32], this.module.f32.add],
        [["*", binaryen.i32, binaryen.i32], this.module.i32.mul],
        [["*", binaryen.f32, binaryen.f32], this.module.f32.mul],
        [["/", binaryen.i32, binaryen.i32], this.module.i32.div_u],
        [["/", binaryen.f32, binaryen.f32], this.module.f32.div],
        [["-", binaryen.i32, binaryen.i32], this.module.i32.sub],
        [["-", binaryen.f32, binaryen.f32], this.module.f32.sub],
    ]);
}
