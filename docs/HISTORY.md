...
- [x] **Erased-vs-packed array buffers — three producers closed + a runtime backstop** (2026-07-24). One failure mode, reached three ways: a buffer allocated **boxed** (24-byte slots, the erased representation) while the call site reads and writes it **packed** (4/8-byte slots, the concrete one). Silent — no diagnostic, no trap — and only past the first buffer growth, which is why each instance cost a self-compile SIGSEGV to find. (1) A **lambda passed to a generic** was checked against an unsubstituted `fn() -> T`, so its `[]` typed `?[]` (see the entry below). (2) **`x := []` widened by its first push** updated `local_types` but left the literal's `expr_types` at `?[]` — and the literal is what the lowerer reads to pick the element kind; `TypedProgram.empty_array_inits` now carries the initialiser's expression id so both widen together. `tests/snippets/array_push` was this exact shape with three pushes and passed by luck; its type table went from `array of ref _` to `array 1`. (3) A **type-param only the result slot can bind** (`rows: i32[] = empty_of()`) was left dangling by *both* inference sites — typecheck gated its expected-type pin on the presence of a lambda argument, and the lowerer's independent `infer_type_args` only ever looked at arguments (GATE B1 "unresolved callee" panic). Both now unify the return type against the call's expected/recorded type, last, so arguments keep priority. **Backstop**: the typed `vader_array_push_*` fast path compares the buffer's element WIDTH against the store's under `#ifndef NDEBUG` and traps — width, not kind identity, since `u64[]` legitimately takes the i64 helper and `char[]` the i32 one, while a boxed buffer is 24 bytes and never matches. Snippets `empty_array_widening` and `generic_return_binds_typaram`, both allowlisted for C-emit parity: native is the oracle here, the VM boxes uniformly and saw none of it.
...
`vader/resolver/body.vader` walks fn bodies / blocks / lambdas / for-loops / if-is-as bindings / match patterns with a scope stack, producing a span-keyed `IdentExpr → Symbol` map (`ResolvedFile.idents`). Symbols carry their kind (`Fn`, `Struct`, `Param`, `Local`, `Binding`, `TypeParam`, …). `vader/lsp/handlers/ast_tokens.vader` runs the positional Phase 1 walker, then a Phase 2 `refine_via_resolver` pass overrides every Phase-1 token whose span matches a resolved ident with the symbol-kind-derived `TokenType`. In-body `x` (param) now paints `Parameter`, `val` (local) paints `Variable`, type names paint `Type`. The resolver lives in `vader/resolver/` so the eventual self-host typechecker (§2.6) reuses the same walker — no duplication between LSP and compiler.
...
- [x] **Float formatting → stdlib (Target ABI follow-up chantier).** `f32`/`f64`
  ...
  out of S1 on purpose. See the S1 decision (2026-06-07).
...
- [x] **Folder-module promotion for single-file entries** (2026-05-18, commit `6fb42f4f`) — what was originally framed as a "transitive module loading" problem turned out to be a folder-module loading bug. When the CLI is invoked on a single `.vader` file (e.g. `./build/vader dump vader/lexer/lexer.vader`), `load_project` used to load that file alone, leaving sibling files in the same folder (`token.vader`, `keywords.vader`, …) unresolved → cascading R2006/T3028/T3001. `load_project` now detects "file inside multi-file folder" and promotes the load root to the parent directory ; `collect_files` merges every file's `CollectedModule` view (previously processed `files[0]` only). Lone-file folders like `stdlib/std/json.vader` keep file-as-module semantics. Wiped ~113 of the 147 errors from the 2026-05-18 audit. Three earlier attempts at transitive eager / on-demand / worklist loading are preserved below for historical record — they're no longer needed for the single-file CLI / LSP use case ; transitive load remains a separate concern for whole-project compile (currently scoped via the snippet harness).
  ...
  - **Attempt 2 (2026-05-18)** : on-demand wire-time loading (`wire_module` calls `load_one` just-in-time when target absent). Also OOMs — even when only 7 mid-graph modules get loaded for a small entry like `vader/lsp/analysis/state.vader`.
...
- [x] **(idea 1) String Hash reads a content-hash cached on the atom** — DONE 2026-06-17.
      ...
      C + VM runs unchanged). Follow-up: **Flavor B** —
