...
| 10 | `f64` value variant : `F64Val` added to the `Value` union ; new ops `F64Const` / `F64Add` / `F64Sub` / `F64Mul` / `F64Div` / `F64Neg` / `F64Eq` / `F64Ne` / `F64Lt` / `F64Le` / `F64Gt` / `F64Ge` with matching exec arms ; `parse_decimal_f64` helper for `f64.const <decimal>` (supports sign, fractional, and exponent parts) ; `std/math` host arms (`sqrt` / `pow` / `floor` / `ceil` / `round` / `abs` / `min` / `max` / `clamp` plus integer-typed `_` variants) routed through `host_f64_unary` / `host_f64_binary` / `host_i32_unary` / `host_i32_binary` helpers ; `std_string$parse_float` host ; `describe_value_tag` extended ; `display_value` extended via `F64Val implements Display`. ✅ | +7 snippets (`expr_bodied_fn`, `json_basics`, `overload_first_param`, `std_math`, `tuple_comptime`, `tuple_struct_field`, `tuple_triple_quad`) — 166/176 = 94 % |
| 12 | Impl table + primitive trait receivers + spawn host : the `.virt` writer emits `impl TYPE_ID TRAIT_NAME` directives from `BytecodeModule.implTable` ; the Vader VM parser records them on `op.Module.impls` and `ref_type_matches` consults them so `is Trait` patterns succeed even when the trait method body got stripped by DCE. `receiver_type_id_of` learns to look up `PrimitiveType.kind` in the module type table (with a primitive-name fallback in `find_struct_type_id`) so traits implemented on primitives (`i32 implements Doubler`) dispatch through the same vtable path. New `std_process$spawn_run` / `spawn_last_stdout` / `spawn_last_stderr` host arms route to the native stdlib intrinsics. ✅ | +4 snippets (`vm_trait_dispatch`, `dot_variant_in_union`, `trait_box_range_iter`, `process_spawn`) — 176/179 = 98 % |
| 11 | `ErrorVal` value variant + `RefType.trait_name` : the `Value` union grows an `ErrorVal { message: string }` variant ; `RefType` learns to carry the trait name from its `.virt` directive ; `ref_type_matches` routes `is Error` patterns to `ErrorVal` checks (and bare `ref _` to "any heap variant", matching `src/vm/exec.ts:matchTo`). `parse_int` / `parse_float` host arms return `ErrorVal` on failure instead of trapping the VM. New `read_file` / `write_file` / `exists` host arms with `Error` propagation through the native stdlib intrinsics (failure path uses a fixed placeholder message — DCE strips most `Error.message()` impls). Three legacy constants (`STDIO_PRINTLN` / `STDIO_PRINT` / `STDIO_EPRINTLN`) inlined at their call sites. ✅ | +3 snippets (`parse_int_match`, `io_roundtrip`, `try_op`) — 172/179 = 96 % |
| 9 | Host-import widening + cross-tag conversions : `string.Hash.hash` (FNV-1a-31-low) ; `<width>.Display.to_string` for every primitive ; `parse_int` / `panic` / `std/runtime` collection-counter hosts (boxed `gc_state`) ; `i32.rem` parser fix (bytecode emits `rem`, not `mod`) ; `IntToChar` / `CharToInt` ops re-tag values across the no-op `Convert` boundary so `${char_val}` interpolation hits the UTF-8 path ; `strip_comment` honours `"..."` boundaries (a literal `;` inside `"...GC; sum=..."` was swallowed) ; `read_i32` / `read_char` accept the cross-tag variant ; `I32ShrU` op for unsigned shifts (`u32.shr`) ; `main` receives a 1-element `argv` array when its signature requires one. ✅ | +24 snippets (`bound_enforced`, `char_range_contains`, `collection_index_sugar`, `format_helpers`, `gc_*` (3), `iter_*` (3), `map_set_iter`, `mutable_*` (3), `path_basics`, `regex_helpers`, `runtime_argv`, `std_cli_basic`, `string_chars`, `string_codepoints`, `trait_dispatch_bounded`, `u32_bitops`, `op_overload_arith`, `op_overload_compound`) — 159/176 = 90 % |
