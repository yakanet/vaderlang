...
| `vader graph <file> [--format=…] [--scope=…]` | ✓       | Print the project's module import graph, read off the loader — so it is the graph a build actually resolves. `--format=mermaid` (default) renders with no tool installed; `--format=dot` pipes to Graphviz. `--scope=project` (default) hides library modules; `--scope=all` keeps them. |
